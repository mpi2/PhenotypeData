			...
			local({
				...
				for (colony_id in colony_ids) {
					...
					for (zygname in group_colony_id_zygosity) {
						...
						#
						dataset_group_colony_zygosity_controls <-
							dataset_group[c(group_colony_id_zygosity, controls_in_group),]

						#
						# Delete any dataset from previous executions
						if ("dataset_phenList" %in% ls(all.names =  TRUE)) {
							rm(dataset_phenList)
						}
						...
						for (depvar in depvars) {
							...
							if (!"dataset_phenList" %in% ls() ||
									class(dataset_phenList) == "try-error") {
								out <-
									paste(
										metadata_group,
										zygname,
										...
										depvar,
										"FAILED",
										"Not enough mutant or control data",
										num_control_male,
										num_control_female,
										num_mutes_male,
										num_mutes_female,
							...
							concurrentContSelect = function(PhenListObj,
								...
								if (is.numeric(PhenListObj@datasetPL[, depVar])) {
									m  = droplevels(PhenListObj@datasetPL[PhenListObj@datasetPL[, GenotypeColName] !=
																													control,])
									m1 =  droplevels(PhenListObj@datasetPL[PhenListObj@datasetPL[, BatchColName] %in% m[, BatchColName],])
							...
							count_female_controls_in_group <-
								length(which(!is.na(
									operational_dataset@datasetPL[which(
										operational_dataset@datasetPL$Sex == "Female" &
											operational_dataset@datasetPL$Genotype == "+/+"
									), ][depvar]
								)))
							count_male_controls_in_group   <-
								length(which(!is.na(
									operational_dataset@datasetPL[which(
										operational_dataset@datasetPL$Sex == "Male" &
											operational_dataset@datasetPL$Genotype == "+/+"
									), ][depvar]
								)))
							count_female_mutants_in_group  <-
								length(which(!is.na(
									operational_dataset@datasetPL[which(
										operational_dataset@datasetPL$Sex == "Female" &
											operational_dataset@datasetPL$Genotype == colony_id
									), ][depvar]
								)))
							count_male_mutants_in_group    <-
								length(which(!is.na(
									operational_dataset@datasetPL[which(
										operational_dataset@datasetPL$Sex == "Male" &
											operational_dataset@datasetPL$Genotype == colony_id
							...
							if (num_mutes_female + num_mutes_male < 1) {
								message(
									...
									":",
									depvar,
									":Not processing - not enough mutant data"
								)
							...
							batches <-
								operational_dataset@datasetPL[which(!is.na(operational_dataset@datasetPL[depvar])),]$batch
							colony_ids_for_strat <-
								operational_dataset@datasetPL[which(!is.na(operational_dataset@datasetPL[depvar])),]$colony_id
							sexes_for_strat <-
								operational_dataset@datasetPL[which(!is.na(operational_dataset@datasetPL[depvar])),]$sex


							starter_female <-
								operational_dataset@datasetPL[which(sexes_for_strat == "female" &
																											colony_ids_for_strat == colony_id),]
							starter_male <-
								operational_dataset@datasetPL[which(sexes_for_strat == "male" &
																											colony_ids_for_strat == colony_id),]
							female_mutant_batches <-
								unique(starter_female[!is.na(starter_female[depvar]),]$batch)
							male_mutant_batches <-
								unique(starter_male[!is.na(starter_male[depvar]),]$batch)
							combined_batches <-
								unique(c(female_mutant_batches, male_mutant_batches))

							if ((length(male_mutant_batches) == 0 &&
									 length(female_mutant_batches) > 0) ||
									(length(female_mutant_batches) == 0 &&
									 length(male_mutant_batches) > 0)) {
							...
							} else if (length(male_mutant_batches) >= 3 &&
												 length(female_mutant_batches) >= 2 ||
												 length(female_mutant_batches) &&
												 length(male_mutant_batches) >= 2) {
								workflow <- "multi_batch"
							}
							...
							if (is.character(dataset_group_colony_zygosity_controls[depvar][1, ])) {
								...
								female_controls_in_group <-
									operational_dataset@datasetPL[which(
										operational_dataset@datasetPL$Sex == "Female" &
											operational_dataset@datasetPL$Genotype == "+/+"
									), depvar]
								male_mutants_in_group <-
									operational_dataset@datasetPL[which(
										operational_dataset@datasetPL$Sex == "Male" &
											operational_dataset@datasetPL$Genotype == colony_id
									), depvar]
								female_mutants_in_group <-
									operational_dataset@datasetPL[which(
										operational_dataset@datasetPL$Sex == "Female" &
											operational_dataset@datasetPL$Genotype == colony_id
									), depvar]
								if (!all(is.na(female_controls_in_group))) {
									mean_control_female = mean(female_controls_in_group, na.rm = TRUE)
								}
								if (!all(is.na(male_controls_in_group))) {
									mean_control_male = mean(male_controls_in_group, na.rm = TRUE)
								}
								if (!all(is.na(female_mutants_in_group))) {
									mean_mutes_female = mean(female_mutants_in_group, na.rm = TRUE)
								}
								if (!all(is.na(male_mutants_in_group))) {
									mean_mutes_male = mean(male_mutants_in_group, na.rm = TRUE)
								}
							}
							...
							message(paste("  got equation ", EQUATION))
							...
							if (all(is.na(operational_dataset@datasetPL[depvar]))) {
								...
								code   <-
									paste(
										"Not enough mutant measurements (",
										num_mutes_female,
										"f/",
										num_mutes_male,
										"m)",
							...
							if (status == STATUS_TESTED) {
								...
								if (num_mutes_female < NUM_MUTANT_FEMALE_THRES &&
										(IS_ABR && num_mutes_female < ABR_THRES)) {
									# Too few females, create a new PhenList with just te males

									operational_dataset <- dataset_phenList
									operational_dataset@datasetPL = operational_dataset@datasetPL[, include_columns]
									operational_dataset <- try(PhenList(
										dataset = operational_dataset@datasetPL[operational_dataset@datasetPL$Sex ==
																															"Male",],
								...
								} else if (num_mutes_male < NUM_MUTANT_MALE_THRES &&
													 (IS_ABR && num_mutes_male < ABR_THRES)) {
								...
								if (nlevels(factor(operational_dataset@datasetPL[[depvar]])) > 1) {
									test001 <-
										capture.stderr(try(testDataset(
											operational_dataset,
											depVariable = depvar,
											method = method01,
											equation = EQUATION
									...
									if (IS_ABR &&
										...
										if (IS_ABR &&
												!identical(grep("jitter", test001$output, fixed  =  TRUE),
																	 integer(0)) && grep("jitter", test001$output, fixed  =  TRUE) > 0)
										{
								...
								if ("test001" %in% ls() &&
										class(test001$result[[1]]$value) == "try-error") {
									status <- STATUS_FAILED
									...
									if (length(index001) > 0) {
										...
										#
										code   <-
											paste(test001$output[index001:index001 + 1], collapse = " ")
									} else {
										# default message
										#
										code   <-
											"testDataset returned an error, check log file"
									}
									...
									#
									output <-
										PhenStat:::vectorOutput(test001$result[[1]]$value, phenotypeThreshold = 0.0001)
							...
							if (status == STATUS_FAILED) {
								...
								#
								output <-
									rep(MISSING_DATA_VALUE, length(vectorOutputNames))
							...
							#
							out <-
								paste(out, paste(output[vectorOutputNames], collapse = "\t"), sep = "\t")
...
launcher <- function() {
	...
	option_list = list(
		...
		make_option(
			c("-e", "--eqfile"),
			type = "character",
			default = "functionEquationMap",
			help = "variable to equation map [default= %default]",
			metavar = "character"
		),
		make_option(
			c("-w", "--withWeight"),
