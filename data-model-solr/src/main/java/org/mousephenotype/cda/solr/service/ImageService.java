...
public class ImageService extends BasicService implements WebStatus {
    ...
    @Inject
    public ImageService(@Qualifier("impcImagesCore") SolrClient impcImagesCore, SolrClient statisticalResultCore) {
        Assert.notNull(impcImagesCore, "ImageService impc_images core cannot be null");
        Assert.notNull(statisticalResultCore, "ImageService statisticalResultCore core cannot be null");
        ...
        if (statisticalResultCore instanceof HttpSolrClient) {
            logger.info("Image Service starting with stats results core: " + ((HttpSolrClient) statisticalResultCore).getBaseURL());
        }
    }
    ...
    /**
     * @param query the url from the page name onwards e.g
...
//					+ mpId+"\"");
    ...
    public Map<String, Set<String>> getImagePropertiesThatHaveMp(String acc) throws SolrServerException, IOException {
        ...
        query.setQuery(StatisticalResultDTO.MARKER_ACCESSION_ID + ":\"" + acc + "\"").setRows(100000000);
        query.addFilterQuery(StatisticalResultDTO.MP_TERM_ID + ":*");
        query.setFacet(true);
        query.setFacetLimit(-1);
        query.setFacetMinCount(1);

        String pivotFacet = StatisticalResultDTO.MP_TERM_ID + "," + StatisticalResultDTO.COLONY_ID;
        query.set("facet.pivot", pivotFacet);
        query.addFacetField(StatisticalResultDTO.COLONY_ID);
    ...
    private SolrDocument fixPdfUrl(SolrDocument impcImageDocument) {
        if(impcImageDocument.get("file_type").toString().equalsIgnoreCase("application/pdf")) {
