...
public class EssentialGeneService extends BasicService implements WebStatus {
    ...
    public List<EssentialGeneDTO> getAllIdgGeneList( String ...fields) throws IOException, SolrServerException {
        ...
        SolrQuery solrQuery = new SolrQuery()
                .setQuery("idg_family:Kinase OR idg_family:IC OR idg_family:GPCR").setRows(Integer.MAX_VALUE);//Integer.MAX_VALUE);//hopefully this gets all the idg results which here is 125147 found instead of 235817 entries for all - changed to idg_family???
        if(fields != null){
            solrQuery.setFields(fields);
        }
    ...
    public List<EssentialGeneDTO> getAllIdgGeneListByGroupLabel(String groupLabel, String ...fields) throws IOException, SolrServerException {
        List<EssentialGeneDTO>idgGeneDTOS = null;
        SolrQuery solrQuery = new SolrQuery()
                .setQuery("idg_family:"+groupLabel).setRows(Integer.MAX_VALUE);//Integer.MAX_VALUE);//hopefully this gets all the idg results which here is 125147 found instead of 235817 entries for all - changed to idg_family???
        if(fields != null){
            solrQuery.setFields(fields);
        }
        //limit it to the 3 families we are currently displaying on the idg page?? should we be doing this? this is query http://ves-ebi-d0.ebi.ac.uk:8986/solr/essentialgenes/select?facet.field=idg_family&facet=on&fq=idg_family:Kinase%20OR%20idg_family:IC%20OR%20idg_family:GPCR&q=*:*
