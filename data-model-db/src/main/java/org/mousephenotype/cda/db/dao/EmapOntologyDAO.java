...
public class EmapOntologyDAO extends OntologyDAO {
    ...
    public final List<List<String>> getDescendentGraphs(String id) {
        ...
        String query =
                "SELECT *\n"
                        + "FROM emap_node_subsumption_fullpath_concat\n"
    ...
    public List<Integer> getNodeIds(String emapTermId){
        return term2NodesMap.get(emapTermId);
    }
    ...
    /**
     * Returns the emap subset for the given id.
     *
     * @param id the emap id to query
     *
     * @return the emap subset for the given id.
     */
    ...
    /**
     * Returns the emap subset for the given id in the provided list.
     *
     * @param id the emap id to query
     *
    ...
    protected void populateAncestorMap() throws SQLException {
        ...
        String query = "SELECT * FROM emap_node_backtrace_fullpath\n";
        ...
        query = "SELECT \n"
                + "  node_id\n"
                + ", top_level_node_id\n"
                + "FROM emap_node_2_selected_top_level_mapping\n";
        ...
        try (final PreparedStatement ps = connection.prepareStatement(query)) {
            ...
            while (resultSet.next()) {
                ...
                if (selectedTopLevelNodeId == 0) {
                    ...
                    logger.warn("node " + node + " in emap_node_2_selected_top_level_mapping has no top-level node.");
                    continue;
                }
    ...
    private int getTopLevelTermIdCount(String termId) {
        ...
        String query =
                "SELECT COUNT(DISTINCT top_level_term_id) AS topLevelTermIdCount\n"
                        + "FROM emap_node_2_selected_top_level_mapping stlm\n"
                        + "WHERE node_id IN (\n"
                        + "        SELECT DISTINCT child_node_id FROM emap_node_subsumption_fullpath sf\n"
                        + "    WHERE sf.node_id IN (\n"
                        + "        SELECT node_id FROM emap_node2term WHERE term_id = ?\n"
                        + "    )\n"
                        + ")\n";
