...
<t:genericpage>
	...
	<jsp:attribute name="breadcrumb">&nbsp;&raquo;&nbsp;<a href="${baseUrl}/searchoverview?">Search Overview</a> &raquo; <a href="${baseUrl}/searchoverview/${dataType}?kw=*">${dataTypeLabel}</a> &raquo; ${searchQuery}</jsp:attribute>
	...
	<jsp:body>
		...
		<script>
			$(document).ready(function(){
				...
				//---------------------- parse URL ----------------------------
				if ( /searchoverview\/?\w*\/?.*$/.exec(location.href) ){
					// with filter(s)

					var regex = /searchoverview\/?(\w*)\/?(.*)$/;
				...
				//---------------------- end of parse URL ----------------------------
					...
					// ----------- highlights current "tab" and populates its facet filters and dataTable -----------
						...
						<%--if ( $.fn.fetchUrlParams('fq') != undefined ){--%>
							<%--$(this).attr('href', baseUrl + '/searchoverview/' + coreName + '?kw=' + query + '&fq=' + $.fn.fetchUrlParams('fq'));--%>
						<%--}--%>
						<%--else {--%>
							...
							<%--}--%>
							<%--else {--%>
								<%--$(this).attr('href', baseUrl + '/searchoverview/' + coreName + '?kw=' + query);--%>
							<%--}--%>
						<%--}--%>
...
//					});
			...
			function activateImgViewSwitcher(){
				$('div#imgView').click(function(){
					...
					if ( mode == 'annotView' ){
						document.location.href = baseUrl + '/searchoverview/' + coreName + "?kw=" + query + fqStr + "&showImgView=true";
					}
					else {
						document.location.href = baseUrl + '/searchoverview/' + coreName + "?kw=" + query + fqStr + "&showImgView=false";
					}
				});
			}
			...
			function fetchResultByFilters(){
				$('li.fcat input').click(function () {
					...
					//document.location.href = baseUrl + '/searchoverview/' + query + '/' + coreName + fqStr;
					document.location.href = baseUrl + '/searchoverview/' + coreName + "?kw=" + queryOri + fqStr;
				});
			}
