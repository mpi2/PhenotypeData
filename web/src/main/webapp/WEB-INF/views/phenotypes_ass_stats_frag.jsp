        ...
        <div class="row no-gutters">
            ...
            <c:if test="${parametersAssociated.size() == 0}">
                ...
                <c:if test="${genePercentage.getTotalGenesTested() > 0}">
                    <p><span class="muchbigger">${genePercentage.getTotalPercentage()}%</span> of tested genes with null
                        mutations on a B6N genetic background have a phenotype association to ${phenotype.getMpTerm()}
        ...
        <div class="row no-gutters">
            ...
            <c:if test="${parametersAssociated.size() > 1}">
                <p> Select a parameter <i class="fa fa-bar-chart-o"></i>&nbsp; &nbsp;
                    <select class="overviewSelect"
                            onchange="ajaxToBe('${phenotype.getMpId()}', this.options[this.selectedIndex].value);">
                        <c:forEach var="assocParam" items="${parametersAssociated}" varStatus="loop">
                            <option value="${assocParam.getStableId()}">${assocParam.getName()}
                                (${assocParam.getStableId()})
                            </option>
                        </c:forEach>
                    </select>
                </p>
                ...
                <c:if test="${parametersAssociated.size() > 0}">
                    <div id="chartsHalfBaseline">
                        ...
                        <div id="baseline-chart-container">
                            <div id="baseline-chart-div" class="baselineChart"
                                 parameter="${parametersAssociated.get(0).getStableId()}" mp="${phenotype.getMpId()}">
                            </div>
            ...
            <c:if test="${parametersAssociated.size() > 0}">
                <div class="col-12 col-md-6 ">
                    ...
                    <div id="chart-container">
                        <div id="single-chart-div" class="oChart" parameter="${parametersAssociated.get(0).getStableId()}"
                             mp="${phenotype.getMpId()}">
                        </div>
