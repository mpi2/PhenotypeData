                                ...
                                <div class="col-12 col-md-12">
                                    <div class="pre-content clear-bg">
                                        <div class="page-content people py-5 white-bg">
                                            <c:if test="${solrImagesError ne null}"><h4>There is an error the image
                                                index is down please contact the IMPC if this error persists</h4></c:if>
                                            <c:if test="${imageCount eq 0}"><h4>There are no images
                                                for ${breadcrumbText}</h4></c:if>
                                            <c:if test="${imageCount ne 0}"><h2 class="title" id="top">${imageCount}
                                                images for ${breadcrumbText}</h2></c:if>
                                            <div class="section">
                                                <div class="inner">
                                                    ...
                                                    <div id="pagination" class="pagination">
                                                        <ul>
                                                            ...
                                                            <c:if test="${start ne 0}">
                                                                <li>
                                                                    <a href='${baseUrl}/images?start=${start-length}&length=${length}&q=${q}${filterQueries}&qf=${qf}&defType=${defType}'>Prev</a>
                                                                </li>
                                                            </c:if>

                                                            <c:forEach begin="0" end="${imageCount}" step="${length}"
                                                                       var="i">
                                                                <li <c:if test="${start eq i}">class="active"</c:if>>
                                                                    <a href='${baseUrl}/images?start=<c:if test="${i ne 0}">${i-1}</c:if><c:if test="${i eq 0}">${i}</c:if>&length=${length}&q=${q}${filterQueries}&qf=${qf}&defType=${defType}'><fmt:formatNumber
                                                                            value="${(i+length)/length}"
                                                            ...
                                                            <c:if test="${start+length lt imageCount}">
                                                                <li><a id="next"
                                                                       href='${baseUrl}/images?start=${start+length}&length=${length}&q=${q}${filterQueries}&qf=${qf}&defType=${defType}'>Next</a>
                                                                </li>
                                                            </c:if>
