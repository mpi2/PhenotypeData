...
<a href="${mediaBaseUrl}/${img.largeThumbnailFilePath}" class="fancybox"
	fullRes="${mediaBaseUrl}/${img.fullResolutionFilePath}"><img
	src="${mediaBaseUrl}/${img.smallThumbnailFilePath}"></a>
<div class="caption">
	...
	<c:forEach var="maTerm" items="${img.annotationTermName}"
		varStatus="status">${maTerm}, </c:forEach>
	<c:if test="${not empty img.genotype}">${img.genotype}, </c:if>
	<c:if test="${not empty img.gender}">${img.gender}, </c:if>
	<c:if test="${not empty img.institute}">
		<c:forEach var="org" items="${img.institute}">${org}</c:forEach>
	</c:if>
	<br>
</div>
