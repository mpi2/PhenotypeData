(function e(t,n,r){function s(o,u){if(!n[o]){if(!t[o]){var a=typeof require=="function"&&require;if(!u&&a)return a(o,!0);if(i)return i(o,!0);var f=new Error("Cannot find module '"+o+"'");throw f.code="MODULE_NOT_FOUND",f}var l=n[o]={exports:{}};t[o][0].call(l.exports,function(e){var n=t[o][1][e];return s(n?n:e)},l,l.exports,e,t,n,r)}return n[o].exports}var i=typeof require=="function"&&require;for(var o=0;o<r.length;o++)s(r[o]);return s})({1:[function(require,module,exports){(function(){'use strict';var d3=require('d3');var AxisGroup=function(renderStartPos,renderEndPos,items){this.renderStartPos=renderStartPos;this.renderEndPos=renderEndPos;for(var index in items){if(items[index].type==='genotype'&&items[index].visible===!1){delete items[index]}}
this.items=items};AxisGroup.prototype={constructor:AxisGroup,getRenderStartPos:function(){return this.renderStartPos},setRenderStartPos:function(position){if(position<0||position>this.groupLength()){position=0}
this.renderStartPos=position},getRenderEndPos:function(){return this.renderEndPos},setRenderEndPos:function(position){if(position>this.groupLength()){position=this.groupLength()}
this.renderEndPos=position},itemAt:function(index){var renderedList=this.keys();var item=renderedList[index];return this.get(item)},get:function(key){return this.items[key]},entries:function(){var keys=Object.keys(this.items);var a=[];for(var i=this.renderStartPos;i<this.renderEndPos;i++){var key=keys[i];var el=this.items[key];a.push(el)}
return a},getItems:function(){var keys=Object.keys(this.items);var a=[];for(var i=this.renderStartPos;i<this.renderEndPos;i++){var key=keys[i];var el=this.items[key];a[key]=el}
return a},keys:function(){var renderedList=this.getItems();return Object.keys(renderedList)},displayLength:function(){return(this.renderEndPos-this.renderStartPos)},groupLength:function(){return Object.keys(this.items).length},position:function(key){var renderedList=this.keys();return renderedList.indexOf(key)},groupIDs:function(){return Object.keys(this.items)},groupEntries:function(){var keys=Object.keys(this.items);var a=[];for(var i=0;i<keys.length;i++){var key=keys[i];var el=this.items[key];a.push(el)}
return a},contains:function(key){if(typeof(this.get(key))!='undefined'){return!0}
else{return!1}},sort:function(by){var temp=this.groupEntries();if(by==='Frequency'){temp.sort(function(a,b){var diff=b.count-a.count;if(diff===0){diff=a.id.localeCompare(b.id)}
return diff})}else if(by==='Frequency and Rarity'){temp.sort(function(a,b){return b.sum-a.sum})}else if(by==='Alphabetic'){temp.sort(function(a,b){var labelA=a.label,labelB=b.label;if(labelA<labelB){return-1}
if(labelA>labelB){return 1}
return 0})}
this.items=[];for(var t in temp){this.items[temp[t].id]=temp[t]}},getScale:function(){var values=this.keys();var scale=d3.scale.ordinal().domain(values).rangeRoundBands([0,values.length]);return scale}};module.exports=AxisGroup}())},{"d3":8}],2:[function(require,module,exports){(function(){'use strict';var $=require('jquery');var Utils=require('./utils.js');var DataLoader=function(serverURL,simSearchQuery,limit){this.serverURL=serverURL;this.simSearchQuery=simSearchQuery;this.qryString='';this.groupsNoMatch=[];this.limit=limit;this.owlsimsData=[];this.origSourceList=[];this.maxMaxIC=0;this.targetData={};this.sourceData={};this.cellData={};this.ontologyCacheLabels=[];this.ontologyCache=[];this.loadedNewTargetGroupItems={};this.postDataLoadCallback=''};DataLoader.prototype={constructor:DataLoader,load:function(qrySourceList,targetGroupList,asyncDataLoadingCallback,limit){this.origSourceList=qrySourceList;this.qryString=this.simSearchQuery.inputItemsString+qrySourceList.join("+");if(typeof(limit)!=='undefined'){this.qryString+=this.simSearchQuery.limitString+limit}
this.postDataLoadCallback=asyncDataLoadingCallback;this.process(targetGroupList,this.qryString)},loadCompareData:function(targetGroup,qrySourceList,geneList,asyncDataLoadingCallback){this.postDataLoadCallback=asyncDataLoadingCallback;this.origSourceList=qrySourceList;this.qryString=this.serverURL+this.simSearchQuery.URL+'/'+qrySourceList.join("+")+'/'+geneList.join(",");var self=this;var jqxhr=$.ajax({url:this.qryString,method:'GET',async:!0,dataType:'json'});jqxhr.done(function(data){if(typeof(data.b)==='undefined'){self.groupsNoMatch.push(targetGroup)}else{self.transform(targetGroup,data)}
self.postDataLoadCallback()});jqxhr.fail(function(){console.log('Ajax error - loadCompareData()')})},loadCompareDataForVendor:function(qrySourceList,targetGroupList,asyncDataLoadingCallback,multiTargetsModeTargetLengthLimit){this.origSourceList=qrySourceList;this.qryString=this.serverURL+this.simSearchQuery.URL+'/'+qrySourceList.join("+")+'/';this.postDataLoadCallback=asyncDataLoadingCallback;this.processDataForVendor(targetGroupList,this.qryString,multiTargetsModeTargetLengthLimit)},processDataForVendor:function(targetGrpList,qryString,multiTargetsModeTargetLengthLimit){if(targetGrpList.length>0){var target=targetGrpList[0];targetGrpList=targetGrpList.slice(1);var listOfListsPerTargetGroup=[];for(var j=0;j<target.entities.length;j++){var eachList=[];for(var k=0;k<target.entities[j].phenotypes.length;k++){eachList.push(target.entities[j].phenotypes[k].id)}
target.entities[j].combinedList=eachList.join('+');listOfListsPerTargetGroup.push(target.entities[j].combinedList)}
var qryStringPerTargetGroup=qryString+listOfListsPerTargetGroup.join();var self=this;var jqxhr=$.ajax({url:qryStringPerTargetGroup,method:'GET',async:!0,dataType:'json'});jqxhr.done(function(data){if(typeof(data.b)==='undefined'){self.groupsNoMatch.push(target.groupName)}else{self.transformDataForVendor(target,data,multiTargetsModeTargetLengthLimit)}
self.processDataForVendor(targetGrpList,self.qryString,multiTargetsModeTargetLengthLimit)});jqxhr.fail(function(){console.log('Ajax error - processDataForVendor()')})}else{this.postDataLoadCallback()}},process:function(targetGrpList,qryString){if(targetGrpList.length>0){var target=targetGrpList[0];targetGrpList=targetGrpList.slice(1);var postData=qryString+this.simSearchQuery.targetSpeciesString+target.groupId;var postFetchCallback=this.postSimsFetchCb;this.postFetch(this.serverURL+this.simSearchQuery.URL,target,targetGrpList,postFetchCallback,postData)}else{this.postDataLoadCallback()}},postFetch:function(url,target,targets,callback,postData){var self=this;console.log('POST:'+url);var jqxhr=$.ajax({url:url,method:'POST',data:postData,async:!0,timeout:60000,dataType:'json'});jqxhr.done(function(data){callback(self,target,targets,data)});jqxhr.fail(function(){console.log('Ajax error - postFetch()')})},postSimsFetchCb:function(self,target,targetGrpList,data){if(data!==null||typeof(data)!=='undefined'){if(typeof(data.b)==='undefined'){self.groupsNoMatch.push(target.groupName)}else{self.owlsimsData[target.groupName]=data;self.transform(target.groupName,data)}}
self.process(targetGrpList,self.qryString)},transform:function(targetGroup,data){if(typeof(data)!=='undefined'&&typeof(data.b)!=='undefined'){console.log("Transforming simsearch data of group: "+targetGroup);if(typeof(data.metadata)!=='undefined'){this.maxMaxIC=data.metadata.maxMaxIC}
if(typeof(this.cellData[targetGroup])==='undefined'){this.cellData[targetGroup]={}}
if(typeof(this.targetData[targetGroup])==='undefined'){this.targetData[targetGroup]={}}
if(typeof(this.sourceData[targetGroup])==='undefined'){this.sourceData[targetGroup]={}}
for(var idx in data.b){var item=data.b[idx];var targetID=Utils.getConceptId(item.id);var targetVal={"id":targetID,"label":item.label,"targetGroup":targetGroup,"type":item.type,"rank":parseInt(idx)+1,"score":item.score.score};if(typeof(this.targetData[targetGroup][targetID])==='undefined'){this.targetData[targetGroup][targetID]={}}
this.targetData[targetGroup][targetID]=targetVal;var matches=data.b[idx].matches;var curr_row,lcs,dataVals;var sourceID_a,currID_b,currID_lcs;if(typeof(matches)!=='undefined'&&matches.length>0){for(var matchIdx in matches){var sum=0,count=0;curr_row=matches[matchIdx];sourceID_a=Utils.getConceptId(curr_row.a.id);currID_b=Utils.getConceptId(curr_row.b.id);currID_lcs=Utils.getConceptId(curr_row.lcs.id);lcs=Utils.normalizeIC(curr_row,this.maxMaxIC);var srcElement=this.sourceData[targetGroup][sourceID_a];if(typeof(srcElement)==='undefined'){count++;sum+=parseFloat(curr_row.lcs.IC);dataVals={"id":sourceID_a,"label":curr_row.a.label,"IC":parseFloat(curr_row.a.IC),"count":count,"sum":sum,"type":"phenotype"};this.sourceData[targetGroup][sourceID_a]=dataVals}else{this.sourceData[targetGroup][sourceID_a].count+=1;this.sourceData[targetGroup][sourceID_a].sum+=parseFloat(curr_row.lcs.IC)}
dataVals={"source_id":sourceID_a,"target_id":targetID,"targetGroup":targetGroup,"value":lcs,"a_IC":curr_row.a.IC,"a_label":curr_row.a.label,"subsumer_id":currID_lcs,"subsumer_label":curr_row.lcs.label,"subsumer_IC":parseFloat(curr_row.lcs.IC),"b_id":currID_b,"b_label":curr_row.b.label,"b_IC":parseFloat(curr_row.b.IC),"type":'cell'};if(typeof(this.cellData[targetGroup][sourceID_a])==='undefined'){this.cellData[targetGroup][sourceID_a]={}}
this.cellData[targetGroup][sourceID_a][targetID]=dataVals}}}}},transformDataForVendor:function(target,data,multiTargetsModeTargetLengthLimit){if(typeof(data)!=='undefined'&&typeof(data.b)!=='undefined'){console.log("Vendor Data transforming...");var targetGroupId=target.groupId;var targetGroup=target.groupName;if(typeof(data.metadata)!=='undefined'){this.maxMaxIC=data.metadata.maxMaxIC}
if(typeof(this.targetData[targetGroup])==='undefined'){this.targetData[targetGroup]={}}
if(typeof(this.sourceData[targetGroup])==='undefined'){this.sourceData[targetGroup]={}}
if(typeof(this.cellData[targetGroup])==='undefined'){this.cellData[targetGroup]={}}
for(var i in data.b){for(var j in target.entities){if(data.b[i].id===target.entities[j].combinedList){data.b[i].label=target.entities[j].label;data.b[i].newid=target.entities[j].id;data.b[i].phenodigmScore=target.entities[j].score;data.b[i].info=target.entities[j].info;break}}}
for(var idx in data.b){var item=data.b[idx];var targetID=target.groupId+item.newid;var targetVal={"id":targetID,"label":item.label,"targetGroup":targetGroup,"type":"genotype","info":item.info,"rank":parseInt(idx)+1,"score":Math.round(item.phenodigmScore.score)};if(typeof(this.targetData[targetGroup][targetID])==='undefined'){this.targetData[targetGroup][targetID]={}}
this.targetData[targetGroup][targetID]=targetVal;var matches=data.b[idx].matches;var curr_row,lcs,dataVals;var sourceID_a,currID_b,currID_lcs;if(typeof(matches)!=='undefined'&&matches.length>0){for(var matchIdx in matches){var sum=0,count=0;curr_row=matches[matchIdx];sourceID_a=Utils.getConceptId(curr_row.a.id);currID_b=Utils.getConceptId(curr_row.b.id);currID_lcs=Utils.getConceptId(curr_row.lcs.id);lcs=Utils.normalizeIC(curr_row,this.maxMaxIC);var srcElement=this.sourceData[targetGroup][sourceID_a];if(typeof(srcElement)==='undefined'){count++;sum+=parseFloat(curr_row.lcs.IC);dataVals={"id":sourceID_a,"label":curr_row.a.label,"IC":parseFloat(curr_row.a.IC),"count":count,"sum":sum,"type":"phenotype"};this.sourceData[targetGroup][sourceID_a]=dataVals}else{this.sourceData[targetGroup][sourceID_a].count+=1;this.sourceData[targetGroup][sourceID_a].sum+=parseFloat(curr_row.lcs.IC)}
dataVals={"source_id":sourceID_a,"target_id":targetID,"targetGroup":targetGroup,"value":lcs,"a_IC":curr_row.a.IC,"a_label":curr_row.a.label,"subsumer_id":currID_lcs,"subsumer_label":curr_row.lcs.label,"subsumer_IC":parseFloat(curr_row.lcs.IC),"b_id":currID_b,"b_label":curr_row.b.label,"b_IC":parseFloat(curr_row.b.IC),"type":'cell'};if(typeof(this.cellData[targetGroup][sourceID_a])==='undefined'){this.cellData[targetGroup][sourceID_a]={}}
this.cellData[targetGroup][sourceID_a][targetID]=dataVals}}}}},transformNewTargetGroupItems:function(targetGroup,data,parentGeneID){if(typeof(data)!=='undefined'&&typeof(data.b)!=='undefined'){console.log("transforming genotype data...");if(typeof(data.metadata)!=='undefined'){this.maxMaxIC=data.metadata.maxMaxIC}
for(var idx in data.b){var item=data.b[idx];var targetID=Utils.getConceptId(item.id);var targetVal={"id":targetID,"label":item.label,"targetGroup":item.taxon.label,"type":item.type,'parentGeneID':parentGeneID,"rank":parseInt(idx)+1,"score":item.score.score,"visible":!0};if(typeof(this.targetData[targetGroup][targetID])==='undefined'){this.targetData[targetGroup][targetID]={}}
this.targetData[targetGroup][targetID]=targetVal;var matches=data.b[idx].matches;var curr_row,lcs,dataVals;var sourceID_a,currID_b,currID_lcs;if(typeof(matches)!=='undefined'&&matches.length>0){for(var matchIdx in matches){var sum=0,count=0;curr_row=matches[matchIdx];sourceID_a=Utils.getConceptId(curr_row.a.id);currID_b=Utils.getConceptId(curr_row.b.id);currID_lcs=Utils.getConceptId(curr_row.lcs.id);lcs=Utils.normalizeIC(curr_row,this.maxMaxIC);if(typeof(this.sourceData[targetGroup])==='undefined'){this.sourceData[targetGroup]={}}
var srcElement=this.sourceData[targetGroup][sourceID_a];if(typeof(srcElement)==='undefined'){count++;sum+=parseFloat(curr_row.lcs.IC);dataVals={"id":sourceID_a,"label":curr_row.a.label,"IC":parseFloat(curr_row.a.IC),"count":count,"sum":sum,"type":"phenotype"};this.sourceData[targetGroup][sourceID_a]=dataVals}else{this.sourceData[targetGroup][sourceID_a].count+=1;this.sourceData[targetGroup][sourceID_a].sum+=parseFloat(curr_row.lcs.IC)}
dataVals={"source_id":sourceID_a,"target_id":targetID,"target_type":'genotype',"targetGroup":item.taxon.label,"value":lcs,"a_IC":curr_row.a.IC,"a_label":curr_row.a.label,"subsumer_id":currID_lcs,"subsumer_label":curr_row.lcs.label,"subsumer_IC":parseFloat(curr_row.lcs.IC),"b_id":currID_b,"b_label":curr_row.b.label,"b_IC":parseFloat(curr_row.b.IC),"type":'cell'};if(typeof(this.cellData[targetGroup][sourceID_a])==='undefined'){this.cellData[targetGroup][sourceID_a]={}}
this.cellData[targetGroup][sourceID_a][targetID]=dataVals}}}}},refresh:function(targetGroup,lazy){var list=[],reloaded=!1;if(lazy){for(var idx in targetGroup){if(this.dataExists(targetGroup[idx].name)===!1){list.push(targetGroup[idx])}}}else{list=targetGroup}
if(list.length>0){this.load(this.origSourceList,list,this.postDataLoadCallback);reloaded=!0}
return reloaded},getFetch:function(self,url,target,callback,finalCallback,parent){console.log('GET:'+url);var jqxhr=$.ajax({url:url,method:'GET',async:!0,dataType:'json',});jqxhr.done(function(data){callback(self,target,data,finalCallback,parent)});jqxhr.fail(function(){console.log('Ajax error - getFetch()')})},getOntology:function(id,ontologyDirection,ontologyDepth,finalCallback,parent){var self=this;var direction=ontologyDirection;var relationship=parent.state.ontologyRelationship;var depth=ontologyDepth;var url=this.serverURL+parent.state.ontologyQuery+id.replace('_',':')+"/"+depth+"/"+direction+"/"+relationship+".json";var cb=this.postOntologyCb;this.getFetch(self,url,id,cb,finalCallback,parent)},getNewTargetGroupItems:function(id,finalCallback,parent){var self=this;var url=this.serverURL+"/gene/"+id.replace('_',':')+"/genotype_list.json";var cb=this.getNewTargetGroupItemsCb;this.getFetch(self,url,id,cb,finalCallback,parent)},getNewTargetGroupItemsCb:function(self,id,results,finalCallback,parent){if(typeof(results.genotype_list)!=='undefined'){if(results.genotype_list.length>0){var unstablePrefix=parent.state.unstableTargetGroupItemPrefix;for(var i in results.genotype_list){for(var k in unstablePrefix){if(results.genotype_list[i].id.indexOf(unstablePrefix[k])===0){results.genotype_list.splice(i,1)}}}
var genotype_list=results.genotype_list.slice(0,parent.state.targetGroupItemExpandLimit);var phenotype_id_list=self.origSourceList.join("+");var genotype_id_list='';for(var i in genotype_list){genotype_id_list+=genotype_list[i].id+","}
if(genotype_id_list.slice(-1)===','){genotype_id_list=genotype_id_list.slice(0,-1)}
var compare_url=self.serverURL+parent.state.compareQuery.URL+'/'+phenotype_id_list+"/"+genotype_id_list;var cb=self.getNewTargetGroupItemsCbCb;self.getFetch(self,compare_url,id,cb,finalCallback,parent)}else{var simsearchResults={};var errorMsg=parent.state.messaging.noAssociatedGenotype;finalCallback(simsearchResults,id,parent,errorMsg)}}},getNewTargetGroupItemsCbCb:function(self,id,results,finalCallback,parent){var genotype_id_list=[];if(typeof(results.b)!=='undefined'){for(var i=0;i<results.b.length;i++){genotype_id_list.push(results.b[i].id.replace(':','_'))}
self.loadedNewTargetGroupItems[id]=genotype_id_list;finalCallback(results,id,parent)}else{var simsearchResults={};var errorMsg=parent.state.messaging.noSimSearchMatchForExpandedGenotype;finalCallback(simsearchResults,id,parent,errorMsg)}},postOntologyCb:function(self,id,results,finalCallback,parent){var ontologyInfo=[];var nodes,edges;if(typeof(results)!=='undefined'){edges=results.edges;nodes=results.nodes;for(var i in nodes){if(!nodes.hasOwnProperty(i)){break}
var lab=self.ontologyCacheLabels[nodes[i].id];if(typeof(lab)=='undefined'||(nodes[i].id!=="MP:0000001"&&nodes[i].id!=="OBO:UPHENO_0001001"&&nodes[i].id!=="OBO:UPHENO_0001002"&&nodes[i].id!=="HP:0000118"&&nodes[i].id!=="HP:0000001")){self.ontologyCacheLabels[nodes[i].id]=Utils.capitalizeString(nodes[i].lbl)}}
for(var j in edges){if(!edges.hasOwnProperty(j)){break}
if(edges[j].obj!=="MP:0000001"&&edges[j].obj!=="OBO:UPHENO_0001001"&&edges[j].obj!=="OBO:UPHENO_0001002"&&edges[j].obj!=="HP:0000118"&&edges[j].obj!=="HP:0000001"){ontologyInfo.push(edges[j])}}}
if(ontologyInfo===null){ontologyInfo={}}
var ontoData={"edges":ontologyInfo,"active":1};self.ontologyCache[id]=ontoData;finalCallback(ontoData,id,parent)},getOntologyLabel:function(id){return this.ontologyCacheLabels[id]},getTargets:function(){return this.targetData},getSources:function(){return this.sourceData},getCellData:function(){return this.cellData},getOntologyCacheLabels:function(){return this.ontologyCacheLabels},dataExists:function(targetGroup){var t=this.cellData[targetGroup]||this.targetData[targetGroup];if(typeof(t)==='undefined'){return!1}
return!0},checkOntologyCache:function(id){return this.ontologyCache[id]}};module.exports=DataLoader}())},{"./utils.js":7,"jquery":11}],3:[function(require,module,exports){(function(){'use strict';var $=require('jquery');var Utils=require('./utils.js');var DataManager=function(dataLoader){this.dataLoader=dataLoader;this.target=this.dataLoader.getTargets();this.source=this.dataLoader.getSources();this.cellData=this.dataLoader.getCellData();this.maxMaxIC=this.dataLoader.maxMaxIC;this.matrix=[];this.reorderedTargetEntriesNamedArray={};this.reorderedTargetEntriesIndexArray={};this.expandedItemList={}};DataManager.prototype={constructor:DataManager,getData:function(dataset,targetGroup){return this[dataset][targetGroup]},appendNewItemsToOrderedTargetList:function(targetGroup,data){var newlyAdded={};for(var i=0;i<data.length;i++){var id=Utils.getConceptId(data[i].id);if(typeof(newlyAdded[targetGroup])==='undefined'){newlyAdded[targetGroup]=[]}
newlyAdded[targetGroup].push(this.target[targetGroup][id])}
if(typeof(this.reorderedTargetEntriesIndexArray[targetGroup])==='undefined'){this.reorderedTargetEntriesIndexArray[targetGroup]=[]}
return this.reorderedTargetEntriesIndexArray[targetGroup].concat(newlyAdded[targetGroup])},updateTargetList:function(genotypesData){var targetEntries=genotypesData.targetEntries;var genotypes=genotypesData.genotypes;var parentGeneID=genotypesData.parentGeneID;var group=genotypesData.group;var gene_position;var first_genotype_position;var header=[];var body=[];var footer=[];for(var i=0;i<targetEntries.length;i++){if(typeof(targetEntries[i])==='undefined'){targetEntries[i]={}}
if(targetEntries[i].id===parentGeneID){gene_position=i;break}}
header=targetEntries.slice(0,gene_position+1);body=targetEntries.slice(header.length,targetEntries.length-genotypes.length);footer=targetEntries.slice(header.length+body.length);var reorderedTargetEntriesIndexArray=header.concat(footer,body);this.reorderedTargetEntriesIndexArray[group]=reorderedTargetEntriesIndexArray;var reorderedTargetEntriesNamedArray={};for(var k=0;k<reorderedTargetEntriesIndexArray.length;k++){if(typeof(reorderedTargetEntriesIndexArray[k])==='undefined'){reorderedTargetEntriesIndexArray[k]={}}
reorderedTargetEntriesNamedArray[reorderedTargetEntriesIndexArray[k].id]=reorderedTargetEntriesIndexArray[k]}
this.reorderedTargetEntriesNamedArray[group]=reorderedTargetEntriesNamedArray},getReorderedTargetEntriesNamedArray:function(group){var t=[]
for(var i=0;i<this.reorderedTargetEntriesIndexArray[group].length;i++){if(typeof(this.reorderedTargetEntriesIndexArray[group][i].visible)==='undefined'){t.push(this.reorderedTargetEntriesIndexArray[group][i])}else{if(this.reorderedTargetEntriesIndexArray[group][i].visible===!0){t.push(this.reorderedTargetEntriesIndexArray[group][i])}}}
var reorderedVisibleTargetEntriesNamedArray={};for(var k=0;k<t.length;k++){reorderedVisibleTargetEntriesNamedArray[t[k].id]=t[k]}
return reorderedVisibleTargetEntriesNamedArray},length:function(dataset,targetGroup){var len=0,a;if(typeof(targetGroup)==='undefined'){a=this[dataset]}else{a=this[dataset][targetGroup]}
if(typeof(a)!=='undefined'){len=Object.keys(a).length}
return len},cellPointMatch:function(key1,key2,targetGroup){var rec;if(typeof(this.cellData[targetGroup])!=='undefined'){if(typeof(this.cellData[targetGroup][key1])!=='undefined'){if(typeof(this.cellData[targetGroup][key1][key2])!=='undefined'){rec=this.cellData[targetGroup][key1][key2]}}else if(typeof(this.cellData[targetGroup][key2])!=='undefined'){if(typeof(this.cellData[targetGroup][key2][key1])!=='undefined'){rec=this.cellData[targetGroup][key2][key1]}}}
return rec},matches:function(key,targetGroup){var matchList=[];var cd=this.cellData;if(typeof(cd[targetGroup])!=='undefined'){if(typeof(cd[targetGroup][key])!=='undefined'){matchList=Object.keys(cd[targetGroup][key]).map(function(k)
{return cd[targetGroup][key][k]})}
else{var srcs=Object.keys(cd[targetGroup]);for(var i in srcs){var src=srcs[i];if(typeof(cd[targetGroup][src])!=='undefined'){if(cd[targetGroup][src][key]!=='undefined'){matchList.push(cd[targetGroup][src][key])}}}}}
return matchList},getTargetSourceElement:function(data){var rec;var key=data.id;rec=this.source[key];if(typeof(rec)==='undefined'){var targetGroup=data.targetGroup;rec=this.target[targetGroup][key]}
return rec},keys:function(dataset){var a=this[dataset];return Object.keys(a)},getElement:function(dataset,key,targetGroup){var el;if(typeof(targetGroup)!=='undefined'){el=this[dataset][targetGroup][key]}else{el=this[dataset][key]}
return el},getCellDetail:function(s,t,targetGroup){var rec;try{rec=this.cellData[targetGroup][s][t]}catch(err){rec=this.cellData[targetGroup][t][s]}
return rec},contains:function(dataset,key,targetGroup){var el=this.getElement(dataset,key,targetGroup);if(typeof(el)!=='undefined'){return!1}
return!0},buildMatrix:function(xvals,yvals,flattened,forCompare){var xvalues=xvals,yvalues=yvals;var matrixFlatten=[];if(!flattened){this.matrix=[]}
for(var y=0;y<yvalues.length;y++){var list=[];for(var x=0;x<xvalues.length;x++){if(typeof(forCompare)!=='undefined'){var targetGroup=forCompare}else{var targetGroup=this._getTargetGroup(yvalues[y],xvalues[x])}
if((typeof(yvalues[y])!=='undefined')&&(typeof(xvalues[x])!=='undefined')){if(typeof(this.cellPointMatch(yvalues[y].id,xvalues[x].id,targetGroup))!=='undefined'){var rec={source_id:yvalues[y].id,target_id:xvalues[x].id,xpos:x,ypos:y,targetGroup:targetGroup,type:'cell'};if(flattened){matrixFlatten.push(rec)}else{list.push(rec)}}}}
if(!flattened){this.matrix.push(list)}}
if(flattened){return matrixFlatten}else{return this.matrix}},getMatrixSourceTargetMatches:function(matchpos,highlightSources){var matchedPositions=[];for(var i in this.matrix){var r=this.matrix[i];for(var j=0;j<r.length;j++){if(r[j].ypos==matchpos&&!highlightSources){matchedPositions.push(r[j])}else if(r[j].xpos==matchpos&&highlightSources){matchedPositions.push(r[j])}}}
return matchedPositions},_getTargetGroup:function(el1,el2){if((typeof(el1)!=='undefined')&&(typeof(el1.targetGroup)!=='undefined')){return el1.targetGroup}
if((typeof(el2)!=='undefined')&&(typeof(el2.targetGroup)!=='undefined')){return el2.targetGroup}},reinitialize:function(targetGroup,lazy){console.log("reinitialize dataManager...");if(this.dataLoader.refresh(targetGroup,lazy)){this.source=[];this.target=[];this.cellData=[];this.target=this.dataLoader.getTargets();this.source=this.dataLoader.getSources();this.cellData=this.dataLoader.getCellData()}},createCombinedTargetList:function(targetGroupList,limit){var combinedTargetList=[];for(var k in targetGroupList){var data=this.getData("target",targetGroupList[k].groupName);if(typeof(data)!=='undefined'){var i=0;for(var idx in data){combinedTargetList[data[idx].id]=data[idx];i++;if(i>=limit){break}}}}
return combinedTargetList},createCombinedSourceList:function(targetGroupList){var combinedSourceList=[];for(var k in targetGroupList){var srcs=this.getData("source",targetGroupList[k].groupName);if(typeof(srcs)!=='undefined'){for(var idx in srcs){var id=srcs[idx].id;var srcData=combinedSourceList[id];if(typeof(srcData)=='undefined'){var newElement={};combinedSourceList[id]=$.extend({},newElement,srcs[idx])}}}}
for(var s in combinedSourceList){combinedSourceList[s].count=0;combinedSourceList[s].sum=0;for(var t in targetGroupList){var cellData=this.getData("cellData",targetGroupList[t].groupName);for(var cd in cellData){var cells=cellData[cd];for(var c in cells){if(combinedSourceList[s].id==cells[c].source_id){combinedSourceList[s].count+=1;combinedSourceList[s].sum+=cells[c].subsumer_IC}}}}}
return combinedSourceList},getOntologyLabel:function(id){return this.dataLoader.getOntologyLabel(id)},isExpanded:function(id){if(typeof(this.expandedItemList[id])==='undefined'){return!1}else{return!0}},checkExpandedItemsLoaded:function(group,id){if(typeof(this.reorderedTargetEntriesIndexArray[group])==='undefined'){this.reorderedTargetEntriesIndexArray[group]=[]}
for(var i=0;i<this.reorderedTargetEntriesIndexArray[group].length;i++){if(typeof(this.reorderedTargetEntriesIndexArray[group][i].parentGeneID)!=='undefined'){if(this.reorderedTargetEntriesIndexArray[group][i].parentGeneID===id){return!0}}}
return!1}};module.exports=DataManager}())},{"./utils.js":7,"jquery":11}],4:[function(require,module,exports){module.exports={"scores":"<div><h5>What is the score shown at the top of the grid?<\/h5><div>The score indicated at the top of each column, below the target label, is the overall similarity score between the query and target. Briefly, for each of the targets (columns) listed, the set of Q(1..n) phenotypes of the query are pairwise compared against all of the T(1..m) phenotypes in the target.<br \/><br \/>Then, for each pairwise comparison of phenotypes (q x P1...n), the best comparison is retained for each q and summed for all p1..n. <\/div><br \/><div>The raw score is then normalized against the maximal possible score, which is the query matching to itself. Therefore, range of scores displayed is 0..100. For more details, please see (Smedley et al, 2012 <a href='http:\/\/www.ncbi.nlm.nih.gov\/pubmed\/23660285' target='_blank'>http:\/\/www.ncbi.nlm.nih.gov\/pubmed\/23660285<\/a> and <a href='http:\/\/www.owlsim.org' target='_blank'>http:\/\/www.owlsim.org<\/a>)<\/div>\r\n","sorts":"<div><h5>What are the different ways that phenotypes can be sorted?<\/h5><div>The phenotypes that are shown on the left side of the grid may be sorted using one of three methods.  More options may be available in the future.<\/div><ul><li><b>Alphabetical<\/b> - A-Z<\/li><li><b>Frequency and Rarity<\/b> - Phenotypes are sorted by the sum of the phenotype values across all models\/genes<\/li><li><b>Frequency (Default)<\/b> - Phenotypes are sorted by the count of the number of model\/gene matches per phenotype <\/li><\/ul>","faq":"<div><h4>Phenogrid Faq<\/h4><h5>How are the similar targets obtained?<\/h5><div>We query our owlsim server to obtain the top 100 most-phenotypically similar targets for the selected organism.  The grid defaults to showing mouse.<\/div><h5>What are the possible targets for comparison?<\/h5><div>Currently, the phenogrid is configured to permit comparisons between your query (typically a set of disease-phenotype associations) and one of:<ul><li>human diseases<\/li><li>mouse genes<\/li><li>zebrafish genes<\/li><\/ul>You can change the target organism by selecting a new organism.  The grid will temporarily disappear, and reappear with the new target rendered.<\/div><h5>Can I compare the phenotypes to human genes?<\/h5><div>No, not yet.  But that will be added soon.<\/div><h5>Where does the data come from?<\/h5><div>The phenotype annotations utilized to compute the phenotypic similarity are drawn from a number of sources:<ul><li>Human disease-phenotype annotations were obtained from  <a href='http:\/\/human-phenotype-ontology.org' target='_blank'>http:\/\/human-phenotype-ontology.org<\/a>, which contains annotations for approx. 7,500 diseases.<\/li><li>Mouse gene-phenotype annotations were obtained from MGI <a href='www.informatics.jax.org'>(www.informatics.jax.org).<\/a> The original annotations were made between genotypes and phenotypes.  We then inferred the relationship between gene and phenotype based on the genes that were variant in each genotype.  We only perform this inference for those genotypes that contain a single variant gene.<\/li><li>Zebrafish genotype-phenotype annotations were obtained from ZFIN  <a href='www.zfin.org' target='_blank'>(www.zfin.org).<\/a> The original annotations were made between genotypes and phenotypes, with some of those genotypes created experimentally with the application of morpholino reagents.  Like for mouse, we inferred the relationship between gene and phenotype based on the genes that were varied in each genotype.  We only perform this inference for those genotypes that contain a single variant gene.<\/li><li>All annotation data, preformatted for use in OWLSim, is available for download from  <a href='http:\/\/code.google.com\/p\/phenotype-ontologies\/' target='_blank'>http:\/\/code.google.com\/p\/phenotype-ontologies\/ <\/a> <\/li><\/ul><h5>What does the phenogrid show?<\/h5><div>The grid depicts the comparison of a set of phenotypes in a query (such as those annotated to a disease or a gene) with one or more phenotypically similar targets.  Each row is a phenotype that is annotated to the query (either directly or it is a less-specific phenotype that is inferred), and each column is an annotated target (such as a gene or disease).  When a phenotype is shared between the query and target, the intersection is colored based on the selected calculation method (see What do the different calculation methods mean).   You can hover over the intersection to get more information about what the original phenotype is of the target, and what is in-common between the two.<\/div><h5>Where can I make suggestions for improvements or additions?<\/h5><div>Please email your feedback to <a href='mailto:info@monarchinitiative.org'>info@monarchinitiative.org.<\/a><h5>What happens to the phenotypes that are not shared?<\/h5><div>Phenotypes that were part of your query but are not shared by any of the targets can be seen by clicking the View Unmatched Phenotype link.<\/div><h5>Why do I sometimes see two targets that share the same phenotypes have very different overall scores?<\/h5><div>This is usually because of some of the phenotypes that are not shared with the query.  For example, if the top hit to a query matches each phenotype exactly, and the next hit matches all of them exactly plus it has 10 additional phenotypes that don&#39;t match it at all, it is penalized for those phenotypes are not in common, and thus ranks lower on the similarity scale. <div><\/div>\r\n","calcs":"<div><h5>What do the different calculation methods mean?<\/h5><div>For each pairwise comparison of phenotypes from the query (q) and target (t), we can assess their individual similarities in a number of ways.  First, we find the phenotype-in-common between each pair (called the lowest common subsumer or LCS). Then, we can leverage the Information Content (IC) of the phenotypes (q,t,lcs) in a variety of combinations to interpret the strength of the similarity.<\/div><br \/><div><b>**Uniqueness <\/b>reflects how often the phenotype-in-common is annotated to all diseases and genes in the Monarch Initiative knowledgebase.  This is simply a reflection of the IC normalized based on the maxIC. IC(PhenotypeInCommon)maxIC(AllPhenotypes)<\/div><br \/><div><b>**Distance<\/b> is the euclidian distance between the query, target, and phenotype-in-common, computed using IC scores.<br\/><center>d=(IC(q)-IC(lcs))2+(IC(t)-IC(lcs))2<\/center>  <\/div><br \/><div>This is normalized based on the maximal distance possible, which would be between two rarely annotated leaf nodes that only have the root node (phenotypic abnormality) in common.  So what is depicted in the grid is 1-dmax(d)<\/div><br \/><div><b>**Ratio(q)<\/b> is the proportion of shared information between a query phenotype and the phenotype-in-common with the target.<br \/><center>ratio(q)=IC(lcs)IC(q)*100<\/center><\/div><br \/><div><b>**Ratio(t)<\/b> is the proportion of shared information between the target phenotype and the phenotype-in-common with the query.<br \/><center>ratio(t)=IC(lcs)IC(t)*100<\/center><\/div><\/div>\r\n"}},{}],5:[function(require,module,exports){module.exports={"logo":"data:image/svg+xml;base64,"}},{}],6:[function(require,module,exports){(function(){'use strict';require('jquery');require('jquery-ui');var d3=require('d3');var filesaver=require('filesaver.js');var AxisGroup=require('./axisgroup.js');var DataLoader=require('./dataloader.js');var DataManager=require('./datamanager.js');var Utils=require('./utils.js');var htmlnotes=require('./htmlnotes.json');var images=require('./images.json');(function(factory){if(typeof module==="object"&&typeof module.exports==="object"){module.exports=factory(require("jquery"),window,document)}else{factory($,window,document)}})(function($,window,document,__undefined__){var createPhenogridForElement=function(element,options){var jqElement=$(element);jqElement.phenogrid(options)};window.Phenogrid={createPhenogridForElement:createPhenogridForElement};$.widget("ui.phenogrid",{config:{serverURL:"https://legacy.monarchinitiative.org",gridSkeletonData:{},selectedCalculation:0,selectedSort:"Frequency",messaging:{misconfig:'Please fix your config to have at least one target group.',gridSkeletonDataError:'No common phenotypes between mouse line and human disease.',noAssociatedGenotype:'This gene has no associated genotypes.',noSimSearchMatchForExpandedGenotype:'No matches found between the provided phenotypes and expanded genotypes.',noSimSearchMatch:'No simsearch matches found for {%groupName%} based on the provided phenotypes.'},gridSkeletonDataVendor:'',owlSimFunction:'',targetSpecies:'',searchResultLimit:100,geneList:[]},internalOptions:{invertAxis:!1,simSearchQuery:{URL:'/simsearch/phenotype',inputItemsString:'input_items=',targetSpeciesString:'&target_species=',limitString:'&limit'},compareQuery:{URL:'/compare'},monarchInitiativeText:'Powered by The Monarch Initiative',unmatchedButtonLabel:'Unmatched Phenotypes',optionsBtnText:'Options',gridTitle:'Phenotype Similarity Comparison',singleTargetModeTargetLengthLimit:30,sourceLengthLimit:30,multiTargetsModeTargetLengthLimit:10,targetLabelCharLimit:23,ontologyDepth:10,ontologyDirection:"OUTGOING",ontologyRelationship:"subClassOf",ontologyQuery:"/neighborhood/",ontologyTreeAmounts:1,targetGroupItemExpandLimit:5,unstableTargetGroupItemPrefix:['MONARCH:','_:'],colorDomains:[0,0.2,0.4,0.6,0.8,1],colorRanges:['rgb(237,248,177)','rgb(199,233,180)','rgb(127,205,187)','rgb(65,182,196)','rgb(29,145,192)','rgb(34,94,168)'],minimap:{x:112,y:75,width:100,height:100,bgColor:'#fff',borderColor:'#666',borderThickness:2,miniCellSize:2,shadedAreaBgColor:'#666',shadedAreaOpacity:0.5},scrollbar:{barToGridMargin:20,barThickness:1,barColor:"#ccc",sliderThickness:8,sliderColor:"#999",},targetGroupDividerLine:{color:"#EA763B",thickness:1,rotatedDividerLength:150},gridRegion:{x:240,y:200,cellPad:19,cellSize:12,rowLabelOffset:25,colLabelOffset:20,scoreOffset:5},gradientRegion:{width:240,height:5},phenotypeSort:["Alphabetic","Frequency and Rarity","Frequency"],similarityCalculation:[{label:"Similarity",calc:0,high:"Max",low:"Min"},{label:"Ratio (q)",calc:1,high:"More Similar",low:"Less Similar"},{label:"Uniqueness",calc:2,high:"Highest",low:"Lowest"},{label:"Ratio (t)",calc:3,high:"More Similar",low:"Less Similar"}]},_destroy:function(){this.element.empty()},_create:function(){if(typeof(configoptions)!=='undefined'){this.configoptions=configoptions}else{this.configoptions={}}
this.state=$.extend({},this.internalOptions,this.config,this.configoptions,this.options);this.state.targetGroupList=[];this.state.initialTargetGroupLoadList=[];this.state.selectedCompareTargetGroup=[];this._createPhenogridContainer();this._showLoadingSpinner();if(this.state.gridSkeletonData.xAxis.length>0&&this.state.gridSkeletonData.yAxis.length>0){if(typeof(this.state.gridSkeletonData.title)!=='undefined'&&this.state.gridSkeletonData.title!==''&&this.state.gridSkeletonData.title!==null){this.state.gridTitle=this.state.gridSkeletonData.title}
this._parseGridSourceList();var self=this;this.state.asyncDataLoadingCallback=function(){self._asyncDataLoadingCB(self)};if(this.state.gridSkeletonDataVendor==='IMPC'){this._initGridSkeletonDataForVendor()}else{if(this.state.owlSimFunction==='compare'&&this.state.geneList.length!==0){this._initCompare()}else if(this.state.owlSimFunction==='search'&&this.state.targetSpecies!==''){this._initSearch()}else{this._initGridSkeletonData()}}}else{this._showGridSkeletonDataErrorMsg()}},_parseGridSourceList:function(){var gridSourceList=[];for(var i=0;i<this.state.gridSkeletonData.yAxis.length;i++){gridSourceList.push(this.state.gridSkeletonData.yAxis[i].id)}
this.state.gridSourceList=this._removeDuplicatedSourceId(gridSourceList)},_initGridSkeletonData:function(){for(var j=0;j<this.state.gridSkeletonData.xAxis.length;j++){this.state.targetGroupList.push(this.state.gridSkeletonData.xAxis[j])}
this._parseTargetGroupList();this.state.dataLoader=new DataLoader(this.state.serverURL,this.state.simSearchQuery);this.state.dataLoader.load(this.state.gridSourceList,this.state.initialTargetGroupLoadList,this.state.asyncDataLoadingCallback)},_initCompare:function(){var compare="compare";this.state.targetGroupList=[{groupName:compare,groupId:compare}];this._parseTargetGroupList();this.state.dataLoader=new DataLoader(this.state.serverURL,this.state.compareQuery);this.state.dataLoader.loadCompareData(this.state.targetGroupList[0].groupName,this.state.gridSourceList,this.state.geneList,this.state.asyncDataLoadingCallback)},_initSearch:function(){if(this.state.targetSpecies==='all'){this.state.targetGroupList=this.state.gridSkeletonData.xAxis;this._parseTargetGroupList()}else{for(var i=0;i<this.state.gridSkeletonData.xAxis.length;i++){if(this.state.gridSkeletonData.xAxis[i].groupId===this.state.targetSpecies){this.state.targetGroupList.push(this.state.gridSkeletonData.xAxis[i]);this._parseTargetGroupList();break}}}
this.state.dataLoader=new DataLoader(this.state.serverURL,this.state.simSearchQuery);this.state.dataLoader.load(this.state.gridSourceList,this.state.initialTargetGroupLoadList,this.state.asyncDataLoadingCallback,this.state.searchResultLimit)},_initGridSkeletonDataForVendor:function(){for(var j=0;j<this.state.gridSkeletonData.xAxis.length;j++){this.state.targetGroupList.push(this.state.gridSkeletonData.xAxis[j])}
this._parseTargetGroupList();this.state.dataLoader=new DataLoader(this.state.serverURL,this.state.compareQuery);if(this._isCrossComparisonView()){this.state.dataLoader.loadCompareDataForVendor(this.state.gridSourceList,this.state.initialTargetGroupLoadList,this.state.asyncDataLoadingCallback,this.state.multiTargetsModeTargetLengthLimit)}else{this.state.dataLoader.loadCompareDataForVendor(this.state.gridSourceList,this.state.initialTargetGroupLoadList,this.state.asyncDataLoadingCallback)}},_showGridSkeletonDataErrorMsg:function(){this.state.pgContainer.html(this.state.messaging.gridSkeletonDataError)},_parseTargetGroupList:function(){for(var idx in this.state.targetGroupList){this.state.initialTargetGroupLoadList.push(this.state.targetGroupList[idx]);this.state.selectedCompareTargetGroup.push(this.state.targetGroupList[idx])}
this._createTargetGroupItemExpansionFlag()},_createTargetGroupItemExpansionFlag:function(){var targetGroupItemExpansionFlag={};for(var i=0;i<this.state.initialTargetGroupLoadList.length;i++){targetGroupItemExpansionFlag[this.state.initialTargetGroupLoadList[i].groupName]=!1}
this.state.expandedTargetGroupItems=$.extend({},this.state.targetGroupItemExpansionFlag);this.state.newTargetGroupItems=$.extend({},this.state.targetGroupItemExpansionFlag);this.state.removedTargetGroupItems=$.extend({},this.state.targetGroupItemExpansionFlag);this.state.reactivateTargetGroupItems=$.extend({},this.state.targetGroupItemExpansionFlag)},_createPhenogridContainer:function(){this.state.pgInstanceId=this.element.attr('id');this.state.pgContainerId=this.state.pgInstanceId+'_container';this.state.pgContainer=$('<div id="'+this.state.pgContainerId+'" class="pg_container"></div>');this.element.append(this.state.pgContainer)},_showLoadingSpinner:function(){var element=$('<div>Loading Phenogrid Widget...<i class="fa fa-spinner fa-pulse"></i></div>');element.appendTo(this.state.pgContainer)},_asyncDataLoadingCB:function(self){self.state.dataManager=new DataManager(self.state.dataLoader);if(self.state.owlSimFunction!=='compare'){self._updateSelectedCompareTargetGroup()}
self.state.pgContainer.html('');if(self.state.owlSimFunction==='compare'){if(this.state.dataLoader.groupsNoMatch.length>0){self._showGroupsNoMatch()}else{self._createDisplay()}}else{self._createDisplay()}},_updateSelectedCompareTargetGroup:function(){for(var i=0;i<this.state.selectedCompareTargetGroup.length;i++){var len=this.state.dataManager.length("target",this.state.selectedCompareTargetGroup[i].groupName);if(typeof(len)==='undefined'||len<1){this.state.selectedCompareTargetGroup.splice(i,1);i--}}},_updateTargetAxisRenderingGroup:function(group_name){var targetList=[];if(this.state.newTargetGroupItems[group_name]){targetList=this.state.dataManager.reorderedTargetEntriesNamedArray[group_name]}else if(this.state.removedTargetGroupItems[group_name]){targetList=this.state.dataManager.getReorderedTargetEntriesNamedArray(group_name)}else if(this.state.reactivateTargetGroupItems[group_name]){targetList=this.state.dataManager.getReorderedTargetEntriesNamedArray(group_name)}else{targetList=this.state.dataManager.getData("target",group_name)}
var targetAxisRenderStartPos=this.state.targetAxis.getRenderStartPos();var targetAxisRenderEndPos=this.state.targetAxis.getRenderEndPos();this.state.targetAxis=new AxisGroup(targetAxisRenderStartPos,targetAxisRenderEndPos,targetList);this._setAxisRenderers()},_createAxisRenderingGroups:function(){var targetList=[],sourceList=[];if(this._isCrossComparisonView()){sourceList=this.state.dataManager.createCombinedSourceList(this.state.selectedCompareTargetGroup);this.state.sourceDisplayLimit=Object.keys(sourceList).length;var targetLengthPerGroup=[];for(var i=0;i<this.state.selectedCompareTargetGroup.length;i++){var targetDataPerGroup=this.state.dataManager.getData('target',this.state.selectedCompareTargetGroup[i].groupName);var groupTargetLength={};if(Object.keys(targetDataPerGroup).length<=this.state.multiTargetsModeTargetLengthLimit){groupTargetLength={groupName:this.state.selectedCompareTargetGroup[i].groupName,targetLength:Object.keys(targetDataPerGroup).length}}else{groupTargetLength={groupName:this.state.selectedCompareTargetGroup[i].groupName,targetLength:this.state.multiTargetsModeTargetLengthLimit}}
targetLengthPerGroup.push(groupTargetLength)}
this.state.targetLengthPerGroup=targetLengthPerGroup;targetList=this.state.dataManager.createCombinedTargetList(this.state.selectedCompareTargetGroup,this.state.multiTargetsModeTargetLengthLimit);this.state.targetDisplayLimit=Object.keys(targetList).length}else if(this.state.selectedCompareTargetGroup.length===1){var singleTargetGroupName=this.state.selectedCompareTargetGroup[0].groupName;sourceList=this.state.dataManager.getData("source",singleTargetGroupName);this.state.sourceDisplayLimit=this.state.dataManager.length("source",singleTargetGroupName);if(this.state.expandedTargetGroupItems[singleTargetGroupName]){targetList=this.state.dataManager.getReorderedTargetEntriesNamedArray(singleTargetGroupName)}else{targetList=this.state.dataManager.getData("target",singleTargetGroupName)}
this.state.targetDisplayLimit=this.state.dataManager.length("target",singleTargetGroupName);if(this.state.targetDisplayLimit>this.state.singleTargetModeTargetLengthLimit){this.state.targetDisplayLimit=this.state.singleTargetModeTargetLengthLimit}}
if(this.state.sourceDisplayLimit>this.state.sourceLengthLimit){this.state.sourceDisplayLimit=this.state.sourceLengthLimit}
this.state.sourceAxis=new AxisGroup(0,this.state.sourceDisplayLimit,sourceList);this.state.sourceAxis.sort(this.state.selectedSort);this.state.targetAxis=new AxisGroup(0,this.state.targetDisplayLimit,targetList);this._setAxisRenderers()},_setAxisRenderers:function(){if(this.state.invertAxis){this.state.xAxisRender=this.state.sourceAxis;this.state.yAxisRender=this.state.targetAxis}else{this.state.xAxisRender=this.state.targetAxis;this.state.yAxisRender=this.state.sourceAxis}},_createDisplay:function(){if(this.state.initialTargetGroupLoadList.length===1){if(this.state.dataLoader.groupsNoMatch.length===0){this._createDisplayComponents()}else{this._showGroupsNoMatch()}}else if(this.state.initialTargetGroupLoadList.length>1){if(this.state.dataLoader.groupsNoMatch.length>0){if(this.state.dataLoader.groupsNoMatch.length===this.state.initialTargetGroupLoadList.length){this._showGroupsNoMatch()}else{this._showGroupsNoMatch();this._createDisplayComponents()}}else{this._createDisplayComponents()}}else{this._showConfigErrorMsg()}},_showConfigErrorMsg:function(){this.state.pgContainer.html(this.state.messaging.misconfig)},_createDisplayComponents:function(){this._createAxisRenderingGroups();this._createColorScalePerSimilarityCalculation();this._createTooltipStub();this._createSvgComponents();this._createUnmatchedSources();this.state.btnPadding=this.state.gridRegion.x-$('#'+this.state.pgInstanceId+'_unmatched_btn').width()-this.state.gridRegion.rowLabelOffset;this._positionUnmatchedSources();this._createPhenogridControls();this._positionPhenogridControls();this._togglePhenogridControls();this._scalableCutoffGroupLabel();this._setSvgSize()},_updateDisplay:function(){this.element.find('#'+this.state.pgInstanceId+'_svg').remove();this._createSvgComponents();this.state.btnPadding=this.state.gridRegion.x-$('#'+this.state.pgInstanceId+'_unmatched_btn').width()-this.state.gridRegion.rowLabelOffset;this._positionUnmatchedSources();this._positionPhenogridControls();this._setSvgSize()},_createSvgComponents:function(){this._createSvgContainer();this._createTargetGroupLabels();this._createNavigation();this._createGrid();this._createScoresTipIcon();this._addGridTitle();this._createGradientLegend();this._createTargetGroupDividerLines();this._createMonarchInitiativeRecognition();this._relinkTooltip()},_createSvgContainer:function(){this.state.pgContainer.append('<svg id="'+this.state.pgInstanceId+'_svg"><g id="'+this.state.pgInstanceId+'_svg_group"></g></svg>');this.state.svg=d3.select('#'+this.state.pgInstanceId+'_svg_group').style("font-family","Verdana, Geneva, sans-serif")},_showGroupsNoMatch:function(){var output='';for(var i=0;i<this.state.dataLoader.groupsNoMatch.length;i++){output+=this.state.messaging.noSimSearchMatch.replace(/{%groupName%}/,this.state.dataLoader.groupsNoMatch[i])+'<br>'}
this.state.pgContainer.append(output)},_addLogoImage:function(){var gridRegion=this.state.gridRegion;var x=(gridRegion.x+this.state.singleTargetModeTargetLengthLimit*gridRegion.cellPad+gridRegion.rowLabelOffset)/2;var self=this;this.state.svg.append("svg:image").attr("xlink:href",images.logo).attr("x",x+$("#"+this.state.pgInstanceId+"_monarchinitiative_text")[0].getBoundingClientRect().width+5).attr("y",this.state.gridRegion.y+this._gridHeight()+74).attr("id",this.state.pgInstanceId+"_logo").attr('class','pg_cursor_pointer').attr("width",40).attr("height",26).on('click',function(){window.open(self.state.serverURL,'_blank')})},_createTargetGroupLabels:function(){if(this.state.owlSimFunction!=='compare'){var self=this;var targetGroupList=this.state.selectedCompareTargetGroup.map(function(d){return d.groupName});var titleXPerGroup=[];var titleYPerGroup=[];if(this._isCrossComparisonView()){var totalColumns=0;var columnsCounter=[];for(var i=0;i<this.state.targetLengthPerGroup.length;i++){totalColumns+=this.state.targetLengthPerGroup[i].targetLength;columnsCounter.push(totalColumns);if(i===0){var x=this.state.gridRegion.x+(this.state.gridRegion.cellPad*columnsCounter[i]+(this.state.targetGroupDividerLine.rotatedDividerLength)*Math.sin(Math.PI/4)-(this.state.gridRegion.cellPad-this.state.gridRegion.cellSize)/2)/2}else{var x=this.state.gridRegion.x+this.state.gridRegion.cellPad*(columnsCounter[i]-this.state.targetLengthPerGroup[i].targetLength)+(this.state.targetGroupDividerLine.rotatedDividerLength)*Math.sin(Math.PI/4)-(this.state.gridRegion.cellPad-this.state.gridRegion.cellSize)/2}
var y=this.state.gridRegion.y+this.state.gridRegion.cellPad*(columnsCounter[i]-this.state.targetLengthPerGroup[i].targetLength/2);titleXPerGroup.push(x);titleYPerGroup.push(y)}}else{var x=this.state.gridRegion.x+this._gridWidth()/2;var y=this.state.gridRegion.y+this._gridHeight()/2;titleXPerGroup.push(x);titleYPerGroup.push(y)}
if(!this.state.invertAxis){this.state.svg.selectAll(".pg_targetGroup_name").data(targetGroupList).enter().append("text").attr("x",function(d,i){return titleXPerGroup[i]}).attr("y",function(d,i){if(i%2===0){return self.state.gridRegion.y-135}else{return self.state.gridRegion.y-135-12}}).attr("class","pg_targetGroup_name").attr("id",function(d,i){return self.state.pgInstanceId+"_groupName_"+(i+1)}).text(function(d,i){return targetGroupList[i]}).style("font-size",'11px').attr("text-anchor",function(d,i){if(i===0){return'middle'}else{return'start'}})}}},_scalableCutoffGroupLabel:function(){if(this._isCrossComparisonView()){for(var i=0;i<this.state.selectedCompareTargetGroup.length;i++){var groupNameWidth=$('#'+this.state.pgInstanceId+' .pg_targetGroup_name')[i].getBoundingClientRect().width;if(i===0){var groupGridWidth=this.state.targetLengthPerGroup[i].targetLength*this.state.gridRegion.cellPad-(this.state.gridRegion.cellPad-this.state.gridRegion.cellSize)/2+(this.state.targetGroupDividerLine.rotatedDividerLength)*Math.sin(Math.PI/4)}else if(i===this.state.selectedCompareTargetGroup.length-1){var groupGridWidth=this.state.targetLengthPerGroup[i].targetLength*this.state.gridRegion.cellPad-(this.state.gridRegion.cellPad-this.state.gridRegion.cellSize)/2-(this.state.targetGroupDividerLine.rotatedDividerLength)*Math.sin(Math.PI/4)+$('#'+this.state.pgInstanceId+'_controls_options').outerWidth()}else{var groupGridWidth=this.state.targetLengthPerGroup[i].targetLength*this.state.gridRegion.cellPad-(this.state.gridRegion.cellPad-this.state.gridRegion.cellSize)/2}
if(groupNameWidth>groupGridWidth){var newCharCount=Math.floor(this.state.selectedCompareTargetGroup[i].groupName.length*(groupGridWidth/groupNameWidth));$('#'+this.state.pgInstanceId+'_groupName_'+(i+1)).text(this.state.selectedCompareTargetGroup[i].groupName.substring(0,newCharCount))}else{if(i===0){$('#'+this.state.pgInstanceId+'_groupName_'+(i+1)).attr("x",this.state.gridRegion.x+(this.state.targetLengthPerGroup[i].targetLength*this.state.gridRegion.cellPad-(this.state.gridRegion.cellPad-this.state.gridRegion.cellSize)/2)/2)}}}}},_createNavigation:function(){var xCount=this.state.xAxisRender.groupLength();var yCount=this.state.yAxisRender.groupLength();var width=this.state.minimap.width;var height=this.state.minimap.height;if(!this.state.invertAxis){if(!this._isCrossComparisonView()){if(yCount>this.state.sourceLengthLimit){if(xCount>this.state.singleTargetModeTargetLengthLimit){this._createMinimap(width,height);this._createScrollbars(!0,!0)}else{width=width*(xCount/this.state.singleTargetModeTargetLengthLimit);this._createMinimap(width,height);this._createScrollbars(!1,!0)}}else{if(xCount>this.state.singleTargetModeTargetLengthLimit){height=height*(yCount/this.state.sourceLengthLimit);this._createMinimap(width,height);this._createScrollbars(!0,!1)}}}else{if(yCount>this.state.sourceLengthLimit){this._createMinimap(width,height);this._createScrollbars(!1,!0)}}}else{if(!this._isCrossComparisonView()){if(xCount>this.state.sourceLengthLimit){if(yCount>this.state.singleTargetModeTargetLengthLimit){this._createMinimap(width,height);this._createScrollbars(!0,!0)}else{height=height*(yCount/this.state.singleTargetModeTargetLengthLimit);this._createMinimap(width,height);this._createScrollbars(!0,!1)}}else{if(yCount>this.state.singleTargetModeTargetLengthLimit){width=width*(xCount/this.state.sourceLengthLimit);this._createMinimap(width,height);this._createScrollbars(!1,!0)}}}else{if(xCount>this.state.sourceLengthLimit){this._createMinimap(width,height);this._createScrollbars(!0,!1)}}}},_createMinimap:function(width,height){var xDisplayCount=this.state.xAxisRender.displayLength();var yDisplayCount=this.state.yAxisRender.displayLength();var xTotalCount=this.state.xAxisRender.groupLength();var yTotalCount=this.state.yAxisRender.groupLength();var overviewX=this.state.minimap.x;var overviewY=this.state.minimap.y;var globalviewGrp=this.state.svg.append("g").attr("id",this.state.pgInstanceId+"_navigator");globalviewGrp.append("rect").attr("x",overviewX).attr("y",overviewY).attr("id",this.state.pgInstanceId+"_globalview").attr("width",width+this.state.minimap.borderThickness*2).attr("height",height+this.state.minimap.borderThickness*2).style("fill",this.state.minimap.bgColor).style("stroke",this.state.minimap.borderColor).style("stroke-width",this.state.minimap.borderThickness);this._createSmallScales(width,height);var xvalues=this.state.xAxisRender.groupEntries();var yvalues=this.state.yAxisRender.groupEntries();if(this.state.owlSimFunction==='compare'){var data=this.state.dataManager.buildMatrix(xvalues,yvalues,!0,this.state.owlSimFunction)}else{var data=this.state.dataManager.buildMatrix(xvalues,yvalues,!0)}
var miniCellsGrp=this.state.svg.select('#'+this.state.pgInstanceId+'_navigator').append('g').attr("id",this.state.pgInstanceId+"_mini_cells_container").attr("transform","translate("+overviewX+","+overviewY+")");var self=this;var cell_rects=miniCellsGrp.selectAll(".mini_cell").data(data,function(d){return d.source_id+d.target_id}).enter().append("rect").attr("class","mini_cell").attr("x",function(d){return self.state.smallXScale(d.target_id)+self.state.minimap.miniCellSize/2}).attr("y",function(d){return self.state.smallYScale(d.source_id)+self.state.minimap.miniCellSize/2}).attr("width",this.state.minimap.miniCellSize).attr("height",this.state.minimap.miniCellSize).attr("fill",function(d){var el=self.state.dataManager.getCellDetail(d.source_id,d.target_id,d.targetGroup);return self._getCellColor(el.value[self.state.selectedCalculation])});var startYId=this.state.yAxisRender.itemAt(0).id;var startXId=this.state.xAxisRender.itemAt(0).id;var selectRectX=this.state.smallXScale(startXId);var selectRectY=this.state.smallYScale(startYId);var selectRectHeight=height*(yDisplayCount/yTotalCount);var selectRectWidth=width*(xDisplayCount/xTotalCount);this.state.highlightRect=this.state.svg.select('#'+this.state.pgInstanceId+'_navigator').append("rect").attr("x",overviewX+selectRectX).attr("y",overviewY+selectRectY).attr("id",this.state.pgInstanceId+"_navigator_shaded_area").attr("height",selectRectHeight+this.state.minimap.borderThickness*2).attr("width",selectRectWidth+this.state.minimap.borderThickness*2).attr("class","pg_draggable").style("fill",this.state.minimap.shadedAreaBgColor).style("opacity",this.state.minimap.shadedAreaOpacity).call(d3.behavior.drag().on("dragstart",self._dragstarted).on("drag",function(){self._crossHairsOff();var newX=parseFloat(d3.select(this).attr("x"))+d3.event.dx;var newY=parseFloat(d3.select(this).attr("y"))+d3.event.dy;if(selectRectHeight===height){newY=overviewY}
if(selectRectWidth===width){newX=overviewX}
if(newX<overviewX){newX=overviewX}
if(newY<overviewY){newY=overviewY}
if(newX+selectRectWidth>overviewX+width){newX=overviewX+width-selectRectWidth}
if(newY+selectRectHeight>overviewY+height){newY=overviewY+height-selectRectHeight}
self.state.svg.select("#"+self.state.pgInstanceId+"_navigator_shaded_area").attr("x",newX).attr("y",newY);self.state.svg.select('#'+self.state.pgInstanceId+'_horizontal_scrollbar_slider').attr("x",function(){var factor=(newX-overviewX)/width;var horizontal_scrollbar_width=self._gridWidth();return self.state.gridRegion.x+horizontal_scrollbar_width*factor});self.state.svg.select('#'+self.state.pgInstanceId+'_vertical_scrollbar_slider').attr("y",function(){var factor=(newY-overviewY)/height;var vertical_scrollbar_height=self._gridHeight();return self.state.gridRegion.y+vertical_scrollbar_height*factor});newX=newX-overviewX;newY=newY-overviewY;var newXPos=self._invertDragPosition(self.state.smallXScale,newX)+xDisplayCount;var newYPos=self._invertDragPosition(self.state.smallYScale,newY)+yDisplayCount;self._updateGrid(newXPos,newYPos)}).on("dragend",self._dragended))},_dragstarted:function(){d3.select(this).classed("pg_dragging",!0)},_dragended:function(){d3.select(this).classed("pg_dragging",!1)},_createScrollbars:function(horizontal,vertical){var self=this;var scrollbar=this.state.scrollbar;var barToGridMargin=scrollbar.barToGridMargin;var barThickness=scrollbar.barThickness;var barColor=scrollbar.barColor;var sliderThickness=scrollbar.sliderThickness;var sliderColor=scrollbar.sliderColor;this._createScrollbarScales(this._gridWidth(),this._gridHeight());var xDisplayCount=this.state.xAxisRender.displayLength();var xTotalCount=this.state.xAxisRender.groupLength();var startXId=this.state.xAxisRender.itemAt(0).id;var defaultX=this.state.gridRegion.x;var sliderRectX=this.state.horizontalScrollbarScale(startXId);var sliderWidth=this._gridWidth()*(xDisplayCount/xTotalCount);var yDisplayCount=this.state.yAxisRender.displayLength();var yTotalCount=this.state.yAxisRender.groupLength();var startYId=this.state.yAxisRender.itemAt(0).id;var defaultY=this.state.gridRegion.y;var sliderRectY=this.state.verticalScrollbarScale(startYId);var sliderHeight=this._gridHeight()*(yDisplayCount/yTotalCount);if(horizontal===!0){var horizontalScrollbarGrp=this.state.svg.append("g").attr("id",this.state.pgInstanceId+"_horizontal_scrollbar_group");horizontalScrollbarGrp.append("line").attr("x1",this.state.gridRegion.x).attr("y1",this.state.gridRegion.y+this._gridHeight()+barToGridMargin).attr("x2",this.state.gridRegion.x+this._gridWidth()).attr("y2",this.state.gridRegion.y+this._gridHeight()+barToGridMargin).attr("id",this.state.pgInstanceId+"_horizontal_scrollbar").style("stroke",barColor).style("stroke-width",barThickness);horizontalScrollbarGrp.append("rect").attr("x",defaultX+sliderRectX).attr("y",this.state.gridRegion.y+this._gridHeight()+barToGridMargin-sliderThickness/2).attr("id",this.state.pgInstanceId+"_horizontal_scrollbar_slider").attr("height",sliderThickness).attr("width",sliderWidth).style("fill",sliderColor).attr("class","pg_draggable").call(d3.behavior.drag().on("dragstart",self._dragstarted).on("drag",function(){self._crossHairsOff();var newX=parseFloat(d3.select(this).attr("x"))+d3.event.dx;if(newX<defaultX){newX=defaultX}
if((newX+sliderWidth)>(self.state.gridRegion.x+self._gridWidth())){newX=self.state.gridRegion.x+self._gridWidth()-sliderWidth}
self.state.svg.select('#'+self.state.pgInstanceId+'_horizontal_scrollbar_slider').attr("x",newX);self.state.svg.select('#'+self.state.pgInstanceId+'_navigator_shaded_area').attr("x",function(){var factor=(newX-defaultX)/self._gridWidth();var minimap_width=parseFloat(d3.select('#'+self.state.pgInstanceId+'_globalview').attr("width"))-2*self.state.minimap.borderThickness;return self.state.minimap.x+minimap_width*factor});newX=newX-defaultX;var newXPos=self._invertDragPosition(self.state.horizontalScrollbarScale,newX)+xDisplayCount;self._updateHorizontalGrid(newXPos)}).on("dragend",self._dragended))}
if(vertical===!0){var verticalScrollbarGrp=this.state.svg.append("g").attr("id",this.state.pgInstanceId+"_vertical_scrollbar_group");verticalScrollbarGrp.append("line").attr("x1",this._positionVerticalScrollbarLine(sliderThickness,barToGridMargin)).attr("y1",this.state.gridRegion.y).attr("x2",this._positionVerticalScrollbarLine(sliderThickness,barToGridMargin)).attr("y2",this.state.gridRegion.y+this._gridHeight()).attr("id",this.state.pgInstanceId+"_vertical_scrollbar").style("stroke",barColor).style("stroke-width",barThickness);verticalScrollbarGrp.append("rect").attr("x",this._positionVerticalScrollbarRect(sliderThickness,barToGridMargin)).attr("y",defaultY+sliderRectY).attr("id",this.state.pgInstanceId+"_vertical_scrollbar_slider").attr("height",sliderHeight).attr("width",sliderThickness).style("fill",sliderColor).attr("class","pg_draggable").call(d3.behavior.drag().on("dragstart",self._dragstarted).on("drag",function(){self._crossHairsOff();var newY=parseFloat(d3.select(this).attr("y"))+d3.event.dy;if(newY<defaultY){newY=defaultY}
if((newY+sliderHeight)>(self.state.gridRegion.y+self._gridHeight())){newY=self.state.gridRegion.y+self._gridHeight()-sliderHeight}
self.state.svg.select('#'+self.state.pgInstanceId+'_vertical_scrollbar_slider').attr("y",newY);self.state.svg.select('#'+self.state.pgInstanceId+'_navigator_shaded_area').attr("y",function(){var factor=(newY-defaultY)/self._gridHeight();var minimap_height=parseFloat(d3.select('#'+self.state.pgInstanceId+'_globalview').attr("height"))-2*self.state.minimap.borderThickness;return self.state.minimap.y+minimap_height*factor});newY=newY-defaultY;var newYPos=self._invertDragPosition(self.state.verticalScrollbarScale,newY)+yDisplayCount;self._updateVerticalGrid(newYPos)}).on("dragend",self._dragended))}},_positionVerticalScrollbarLine:function(sliderThickness,barToGridMargin){var x;if(this._isCrossComparisonView()){x=this.state.gridRegion.x+this.state.multiTargetsModeTargetLengthLimit*this.state.selectedCompareTargetGroup.length*this.state.gridRegion.cellPad-sliderThickness/2+barToGridMargin}else{x=this.state.gridRegion.x+this.state.singleTargetModeTargetLengthLimit*this.state.gridRegion.cellPad-sliderThickness/2+barToGridMargin}
return x},_positionVerticalScrollbarRect:function(sliderThickness,barToGridMargin){var x;if(this._isCrossComparisonView()){x=this.state.gridRegion.x+this.state.multiTargetsModeTargetLengthLimit*this.state.selectedCompareTargetGroup.length*this.state.gridRegion.cellPad+barToGridMargin-sliderThickness}else{x=this.state.gridRegion.x+this.state.singleTargetModeTargetLengthLimit*this.state.gridRegion.cellPad+barToGridMargin-sliderThickness}
return x},_createScrollbarScales:function(width,height){var sourceList=this.state.yAxisRender.groupIDs();var targetList=this.state.xAxisRender.groupIDs();this.state.verticalScrollbarScale=d3.scale.ordinal().domain(sourceList.map(function(d){return d})).rangePoints([0,height]);this.state.horizontalScrollbarScale=d3.scale.ordinal().domain(targetList.map(function(d){return d})).rangePoints([0,width])},_setSvgSize:function(){var svgWidth;if(this._isCrossComparisonView()){svgWidth=this.state.gridRegion.x+this.state.multiTargetsModeTargetLengthLimit*this.state.selectedCompareTargetGroup.length*this.state.gridRegion.cellPad+this.state.gridRegion.rowLabelOffset+$('#'+this.state.pgInstanceId+'_slide_btn').width()+this.state.btnPadding}else{svgWidth=this.state.gridRegion.x+this.state.singleTargetModeTargetLengthLimit*this.state.gridRegion.cellPad+this.state.gridRegion.rowLabelOffset+$('#'+this.state.pgInstanceId+'_slide_btn').width()+this.state.btnPadding}
d3.select('#'+this.state.pgInstanceId+'_svg').attr('width',svgWidth).attr('height',this.state.gridRegion.y+this._gridHeight()+100)},_togglePhenogridControls:function(){var self=this;$('#'+this.state.pgInstanceId+'_slide_btn').click(function(){if(!$(this).hasClass("pg_slide_open")){$("#"+self.state.pgInstanceId+"_controls_options").fadeIn();$(this).addClass("pg_slide_open")}});$('#'+this.state.pgInstanceId+'_controls_close').click(function(){$('#'+self.state.pgInstanceId+'_controls_options').fadeOut();$('#'+self.state.pgInstanceId+'_slide_btn').removeClass("pg_slide_open")})},_toggleUnmatchedSources:function(){var self=this;$('#'+this.state.pgInstanceId+'_unmatched_btn').click(function(){if(!$(this).hasClass("pg_unmatched_open")){$('#'+self.state.pgInstanceId+'_unmatched_list').fadeIn();$(this).addClass("pg_unmatched_open")}});$('#'+this.state.pgInstanceId+'_unmatched_close').click(function(){$('#'+self.state.pgInstanceId+'_unmatched_list').fadeOut();$('#'+self.state.pgInstanceId+'_unmatched_btn').removeClass("pg_unmatched_open")})},_crossHairsOff:function(){this.state.svg.selectAll(".pg_focusLine").remove()},_crossHairsOn:function(id,ypos,direction){var xScale=this.state.xAxisRender.getScale();var xs=xScale(id);var gridRegion=this.state.gridRegion;var x=gridRegion.x+(xs*gridRegion.cellPad)+5;var y=gridRegion.y+(ypos*gridRegion.cellPad)+5;if(direction==='vertical'){this._createFocusLineVertical(x,gridRegion.y,x,gridRegion.y+this._gridHeight())}else if(direction==='horizontal'){this._createFocusLineHorizontal(gridRegion.x,y,gridRegion.x+this._gridWidth(),y)}else{this._createFocusLineVertical(x,gridRegion.y,x,gridRegion.y+gridRegion.cellPad*ypos);this._createFocusLineVertical(x,gridRegion.y+gridRegion.cellPad*ypos+gridRegion.cellSize,x,gridRegion.y+this._gridHeight());this._createFocusLineHorizontal(gridRegion.x,y,gridRegion.x+gridRegion.cellPad*xs,y);this._createFocusLineHorizontal(gridRegion.x+gridRegion.cellPad*xs+gridRegion.cellSize,y,gridRegion.x+this._gridWidth(),y)}},_createFocusLineVertical:function(x1,y1,x2,y2){this.state.svg.append('line').attr('class','pg_focusLine').attr('x1',x1).attr('y1',y1).attr('x2',x2).attr('y2',y2)},_createFocusLineHorizontal:function(x1,y1,x2,y2){this.state.svg.append('line').attr('class','pg_focusLine').attr('x1',x1).attr('y1',y1).attr('x2',x2).attr('y2',y2)},_mouseover:function(elem,d){this._showEffectsOnMouseover(elem,d);var data;if(d.type==='cell'){data=this.state.dataManager.getCellDetail(d.source_id,d.target_id,d.targetGroup)}else{data=d}
this._createHoverBox(data);this._showTooltip($('#'+this.state.pgInstanceId+'_tooltip'),elem,d)},_mouseout:function(){this._removeMatchingHighlight();this._crossHairsOff()},_showEffectsOnMouseover:function(elem,d){if(d.type==='cell'){d3.select("#"+this.state.pgInstanceId+"_grid_row_"+d.ypos+" text").classed("pg_active",!0);d3.select("#"+this.state.pgInstanceId+"_grid_col_"+d.xpos+" text").classed("pg_active",!0);d3.select("#"+this.state.pgInstanceId+"_cell_"+d.ypos+"_"+d.xpos).classed("pg_rowcolmatch",!0);this._crossHairsOn(d.target_id,d.ypos,'both')}else if(d.type==='phenotype'){this._createMatchingHighlight(elem,d);if(!this.state.invertAxis){var yScale=this.state.yAxisRender.getScale();var ypos=yScale(d.id);this._crossHairsOn(d.id,ypos,'horizontal')}else{var xScale=this.state.xAxisRender.getScale();var xpos=xScale(d.id);this._crossHairsOn(d.id,xpos,'vertical')}}else{this._createMatchingHighlight(elem,d);if(!this.state.invertAxis){var xScale=this.state.xAxisRender.getScale();var xpos=xScale(d.id);this._crossHairsOn(d.id,xpos,'vertical')}else{var yScale=this.state.yAxisRender.getScale();var ypos=yScale(d.id);this._crossHairsOn(d.id,ypos,'horizontal')}}},_removeMatchingHighlight:function(){d3.selectAll(".row text").classed("pg_active",!1);d3.selectAll(".column text").classed("pg_active",!1);d3.selectAll(".row text").classed("pg_related_active",!1);d3.selectAll(".column text").classed("pg_related_active",!1);d3.selectAll(".cell").classed("pg_rowcolmatch",!1)},_createMatchingHighlight:function(elem,data){var hightlightSources=!0;var currenPos=this._getAxisDataPosition(data.id);var nameId=elem.parentNode.id;if(nameId.indexOf('grid_col')>-1){hightlightSources=!0;var matches=this.state.dataManager.getMatrixSourceTargetMatches(currenPos,hightlightSources);if(typeof(matches)!=='undefined'){for(var k=0;k<matches.length;k++){d3.select("#"+this.state.pgInstanceId+"_grid_row_"+matches[k].ypos+" text").classed("pg_related_active",!0)}}
d3.select("#"+this.state.pgInstanceId+"_grid_col_"+currenPos+" text").classed("pg_active",!0)}else{hightlightSources=!1;var matches=this.state.dataManager.getMatrixSourceTargetMatches(currenPos,hightlightSources);if(typeof(matches)!=='undefined'){for(var k=0;k<matches.length;k++){d3.select("#"+this.state.pgInstanceId+"_grid_col_"+matches[k].xpos+" text").classed("pg_related_active",!0)}}
d3.select("#"+this.state.pgInstanceId+"_grid_row_"+currenPos+" text").classed("pg_active",!0)}},_gridWidth:function(){var gridRegion=this.state.gridRegion;var gridWidth=(gridRegion.cellPad*this.state.xAxisRender.displayLength())-(gridRegion.cellPad-gridRegion.cellSize);return gridWidth},_gridHeight:function(){var gridRegion=this.state.gridRegion;var height=(gridRegion.cellPad*this.state.yAxisRender.displayLength())-(gridRegion.cellPad-gridRegion.cellSize);return height},_createTextScores:function(){var self=this;var gridRegion=self.state.gridRegion;var list,scale,axRender;if(self.state.invertAxis){list=self.state.yAxisRender.keys();scale=self.state.yAxisRender.getScale();axRender=self.state.yAxisRender}else{list=self.state.xAxisRender.keys();scale=self.state.xAxisRender.getScale();axRender=self.state.xAxisRender}
var scores=this.state.svg.selectAll(".scores").data(list).enter().append("g").attr("class","pg_score_text");scores.append("text").attr("text-anchor","start").style("font-size","9px").style("fill","#8763A3").text(function(d,i){var el=axRender.itemAt(i);return el.score});if(self.state.invertAxis){scores.attr("transform",function(d){return"translate("+(gridRegion.x-gridRegion.cellPad)+", "+(gridRegion.y+scale(d)*gridRegion.cellPad+9)+")"})}else{scores.attr("transform",function(d){return"translate("+(gridRegion.x+scale(d)*gridRegion.cellPad)+","+(gridRegion.y-gridRegion.scoreOffset)+")"})}},_getCellColor:function(score){var selectedScale=this.state.colorScale[this.state.selectedCalculation];return selectedScale(score)},_createScoresTipIcon:function(){var self=this;this.state.svg.append("text").attr('font-family','FontAwesome').text(function(){return'\uF05A\n'}).attr("id",this.state.pgInstanceId+"_scores_tip_icon").attr("class","pg_scores_tip_icon").attr("x",this.state.gridRegion.x-21).attr("y",this.state.gridRegion.y-5).on("click",function(){self._populateDialog(htmlnotes.scores)})},_createSmallScales:function(width,height){var sourceList=this.state.yAxisRender.groupIDs();var targetList=this.state.xAxisRender.groupIDs();this.state.smallYScale=d3.scale.ordinal().domain(sourceList.map(function(d){return d})).rangePoints([0,height]);this.state.smallXScale=d3.scale.ordinal().domain(targetList.map(function(d){return d})).rangePoints([0,width])},_invertDragPosition:function(scale,value){var leftEdges=scale.range();var size=scale.rangeBand();var j;for(j=0;value>(leftEdges[j]+size);j++){}
return j},_getAxisData:function(key){key=key.replace(":","_");if(this.state.yAxisRender.contains(key)){return this.state.yAxisRender.get(key)}else if(this.state.xAxisRender.contains(key)){return this.state.xAxisRender.get(key)}else{return null}},_getAxisDataPosition:function(key){if(this.state.yAxisRender.contains(key)){return this.state.yAxisRender.position(key)}else if(this.state.xAxisRender.contains(key)){return this.state.xAxisRender.position(key)}else{return-1}},_createColorScalePerSimilarityCalculation:function(){this.state.colorScale=[];var len=this.state.similarityCalculation.length;for(var i=0;i<len;i++){var maxScore=100;if(i===2){maxScore=this.state.dataManager.maxMaxIC}
var cs=d3.scale.linear();cs.domain([0,maxScore]);cs.domain(this.state.colorDomains.map(cs.invert));cs.range(this.state.colorRanges);this.state.colorScale[i]=cs}},_createTooltipStub:function(){var pg_tooltip=$("<div>").attr("id",this.state.pgInstanceId+'_tooltip').attr("class",'pg_tooltip');var pg_tooltip_inner=$("<div>").attr("id",this.state.pgInstanceId+"_tooltip_inner").attr("class",'pg_tooltip_inner');pg_tooltip.append(pg_tooltip_inner);this.state.pgContainer.append(pg_tooltip);this._hideTooltip(pg_tooltip)},_relinkTooltip:function(){var self=this;$(document).ready(function($){var $targets=$("*[data-tooltip]");var $tooltip=$('#'+self.state.pgInstanceId+'_tooltip');if($targets.length===0){return}
self._hideTooltip($tooltip);$targets.mouseout(function(e){var elem=e.relatedTarget||e.toElement||e.fromElement;if(typeof(elem)!=='undefined'){if(elem.id!==(self.state.pgInstanceId+'_tooltip')&&elem.id!==""){self._hideTooltip($tooltip)}}})})},_showTooltip:function(tooltip,elem,d){var pos=$(elem).offset();var pgContainerPos=this.state.pgContainer.offset();var leftPos=pos.left-pgContainerPos.left;var topPos=pos.top-pgContainerPos.top;if(elem.parentNode.id.indexOf('grid_row')>-1){leftPos+=elem.getBoundingClientRect().width;topPos+=elem.getBoundingClientRect().height/2}else{leftPos+=10}
var position={left:leftPos,top:topPos};tooltip.css({left:position.left,top:position.top});tooltip.show();this._off(tooltip,"mouseover");this._off(tooltip,"mouseleave");this._on(tooltip,{"mouseover":function(){this._showEffectsOnMouseover(elem,d)}});this._on(tooltip,{"mouseleave":function(){this._hideTooltip(tooltip);this._mouseout()}})},_hideTooltip:function(tooltip){tooltip.hide()},_addGridTitle:function(){if(this.state.gridTitle!==''){var self=this;this.state.svg.append("svg:text").attr("id",this.state.pgInstanceId+"_toptitle").attr("x",function(){if(self._isCrossComparisonView()){return self.state.gridRegion.x+self.state.multiTargetsModeTargetLengthLimit*self.state.selectedCompareTargetGroup.length*self.state.gridRegion.cellPad/2}else{return self.state.gridRegion.x+self.state.singleTargetModeTargetLengthLimit*self.state.gridRegion.cellPad/2}}).attr("y",25).style('text-anchor','middle').style('font-size','1.4em').style('font-weight','bold').text(this.state.gridTitle)}},_createHoverBox:function(data){var id;if(data.type==='cell'){if(this.state.invertAxis){id=data.target_id}else{id=data.source_id}}else{id=data.id}
var retData=this._renderTooltip(id,data);$("#"+this.state.pgInstanceId+"_tooltip_inner").empty();$("#"+this.state.pgInstanceId+"_tooltip_inner").html(retData);if(data.type==='phenotype'){var expandOntol_icon=$('#'+this.state.pgInstanceId+'_expandOntology_'+id);this._on(expandOntol_icon,{"click":function(event){this._expandOntology(id)}})}
if(data.type==='gene'){var insert=$('#'+this.state.pgInstanceId+'_insert_genotypes_'+id);this._on(insert,{"click":function(event){this._insertExpandedItems(id)}});var remove=$('#'+this.state.pgInstanceId+'_remove_genotypes_'+id);this._on(remove,{"click":function(event){this._removeExpandedItems(id)}})}},_phenotypeTooltip:function(id,data){var htmlContent='';var tooltipType=(typeof(data.type)!=='undefined'?"<strong>"+Utils.capitalizeString(data.type)+": </strong> "+this._encodeTooltipHref(data.type,id,data.label)+"<br>":"");var ic=(typeof(data.IC)!=='undefined'?"<strong>IC:</strong> "+data.IC.toFixed(2)+"<br>":"");var sum=(typeof(data.sum)!=='undefined'?"<strong>Sum:</strong> "+data.sum.toFixed(2)+"<br>":"");var frequency=(typeof(data.count)!=='undefined'?"<strong>Frequency:</strong> "+data.count+"<br>":"");htmlContent=tooltipType+ic+sum+frequency;var expanded=!1;var ontologyData="<br>";var cached=this.state.dataLoader.checkOntologyCache(id);if(typeof(cached)!=='undefined'){expanded=!0;this.state.ontologyTreesDone=0;this.state.ontologyTreeHeight=0;var tree='<div id="'+this.state.pgInstanceId+'_hpoDiv">'+this._buildOntologyTree(id.replace("_",":"),cached.edges,0)+'</div>';if(tree==="<br>"){ontologyData+="<em>No Classification hierarchy Found</em>"}else{ontologyData+="<strong>Classification hierarchy:</strong>"+tree}}
if(expanded){htmlContent+=ontologyData}else{htmlContent+='<br><div class="pg_expand_ontology" id="'+this.state.pgInstanceId+'_expandOntology_'+id+'">Expand classification hierarchy<i class="pg_expand_ontology_icon fa fa-plus-circle pg_cursor_pointer"></i></div>'}
return htmlContent},_cellTooltip:function(id,data){var htmlContent='';var suffix="";var selCalc=this.state.selectedCalculation;var prefix,targetId,sourceId,targetInfo,sourceInfo;sourceId=data.source_id;targetId=data.target_id;if(this.state.invertAxis){targetInfo=this.state.yAxisRender.get(data.target_id);sourceInfo=this.state.xAxisRender.get(data.source_id)}else{targetInfo=this.state.xAxisRender.get(data.target_id);sourceInfo=this.state.yAxisRender.get(data.source_id)}
for(var idx in this.state.similarityCalculation){if(this.state.similarityCalculation[idx].calc===this.state.selectedCalculation){prefix=this.state.similarityCalculation[idx].label;break}}
if(selCalc!==2){suffix='%'}
htmlContent="<strong>"+Utils.capitalizeString(sourceInfo.type)+"</strong><br>"+this._encodeTooltipHref(sourceInfo.type,sourceId,data.a_label)+" "+Utils.formatScore(data.a_IC.toFixed(2))+"<br><br>"+"<strong>In-common</strong><br>"+this._encodeTooltipHref(sourceInfo.type,data.subsumer_id,data.subsumer_label)+" ("+Utils.formatScore(data.subsumer_IC.toFixed(2))+", "+prefix+" "+data.value[this.state.selectedCalculation].toFixed(2)+'%'+")<br><br>"+"<strong>Match</strong><br>"+this._encodeTooltipHref(sourceInfo.type,data.b_id,data.b_label)+Utils.formatScore(data.b_IC.toFixed(2));return htmlContent},_vendorTooltip:function(id,data){var htmlContent='';for(var idx in data.info){if(typeof(data.info[idx].id)!==null){if(typeof(data.info[idx].href)==='string'){htmlContent+='<strong>'+data.info[idx].id+'</strong> '+'<a href="'+data.info[idx].href+'" target="_blank">'+data.info[idx].value+'</a>'+'<br>'}else{htmlContent+='<strong>'+data.info[idx].id+'</strong> '+data.info[idx].value+'<br>'}}else{if(typeof(data.info[idx].href)==='string'){htmlContent+='<a href="'+data.info[idx].href+'" target="_blank">'+data.info[idx].value+'</a>'+'<br>'}else{htmlContent+=data.info[idx].value+'<br>'}}}
return htmlContent.slice(0,-4)},_defaultTooltip:function(id,data){var htmlContent='';var tooltipType=(typeof(data.type)!=='undefined'?"<strong>"+Utils.capitalizeString(data.type)+": </strong> "+this._encodeTooltipHref(data.type,id,data.label)+"<br>":"");var rank=(typeof(data.rank)!=='undefined'?"<strong>Rank:</strong> "+data.rank+"<br>":"");var score=(typeof(data.score)!=='undefined'?"<strong>Score:</strong> "+data.score+"<br>":"");var group=(typeof(data.targetGroup)!=='undefined'?"<strong>Species:</strong> "+data.targetGroup+"<br>":"");htmlContent=tooltipType+rank+score+group;if(data.type==='gene'){if(this.state.selectedCompareTargetGroup.length===1&&this.state.selectedCompareTargetGroup[0].groupName!=='compare'){var expanded=this.state.dataManager.isExpanded(id);if(expanded){htmlContent+='<br><div class="pg_expand_genotype" id="'+this.state.pgInstanceId+'_remove_genotypes_'+id+'">Remove associated genotypes<i class="pg_expand_genotype_icon fa fa-minus-circle pg_cursor_pointer"></i></div>'}else{htmlContent+='<br><div class="pg_expand_genotype" id="'+this.state.pgInstanceId+'_insert_genotypes_'+id+'">Insert associated genotypes<i class="pg_expand_genotype_icon fa fa-plus-circle pg_cursor_pointer"></i></div>'}}}
return htmlContent},_renderTooltip:function(id,data){var htmlContent='';if(data.type==='phenotype'){htmlContent=this._phenotypeTooltip(id,data)}else if(data.type==='cell'){htmlContent=this._cellTooltip(id,data)}else if(this.state.gridSkeletonDataVendor==='IMPC'){htmlContent=this._vendorTooltip(id,data)}else{htmlContent=this._defaultTooltip(id,data)}
return htmlContent},_encodeTooltipHref:function(type,id,label){return'<a href="'+this.state.serverURL+'/'+type+'/'+id+'" target="_blank">'+Utils.encodeHtmlEntity(label)+'</a>'},_buildOntologyTree:function(id,edges,level){var results="";var nextResult;var nextLevel=level+1;for(var j in edges){if(!edges.hasOwnProperty(j)){break}
if(edges[j].pred==="subClassOf"&&this.state.ontologyTreesDone!==this.state.ontologyTreeAmounts){if(edges[j].sub===id){if(this.state.ontologyTreeHeight<nextLevel){this.state.ontologyTreeHeight++}
nextResult=this._buildOntologyTree(edges[j].obj,edges,nextLevel);if(nextResult===""){results+="<br>"+this._buildIndentMark(this.state.ontologyTreeHeight-nextLevel)+"<strong>"+this._buildOntologyHyperLink(edges[j].obj)+"</strong>";this.state.ontologyTreesDone++}else{results+=nextResult+"<br>"+this._buildIndentMark(this.state.ontologyTreeHeight-nextLevel)+this._buildOntologyHyperLink(edges[j].obj)}
if(level===0){results+="<br>"+this._buildIndentMark(this.state.ontologyTreeHeight)+this.state.dataManager.getOntologyLabel(id)+"<br>";this.state.ontologyTreeHeight=0}}}}
return results},_buildIndentMark:function(treeHeight){var indent='<em class="pg_ontology_tree_indent"></em>';if(treeHeight===0){return indent}
for(var i=1;i<treeHeight;i++){indent+='<em class="pg_ontology_tree_indent"></em>'}
return indent+'&#8627'},_buildOntologyHyperLink:function(id){return'<a href="'+this.state.serverURL+'/phenotype/'+id+'" target="_blank">'+this.state.dataManager.getOntologyLabel(id)+'</a>'},_createTargetGroupDividerLines:function(){var gridRegion=this.state.gridRegion;if(this._isCrossComparisonView()){var totalColumns=0;var columnsCounter=[];for(var i=0;i<this.state.targetLengthPerGroup.length-1;i++){totalColumns+=this.state.targetLengthPerGroup[i].targetLength;columnsCounter.push(totalColumns)}
for(var i=1;i<this.state.selectedCompareTargetGroup.length;i++){if(this.state.invertAxis){var y=gridRegion.y+gridRegion.cellPad*columnsCounter[i-1]-(gridRegion.cellPad-gridRegion.cellSize)/2;this.state.svg.append("line").attr("class","pg_target_grp_divider").attr("x1",gridRegion.x-gridRegion.rowLabelOffset).attr("y1",y).attr("x2",gridRegion.x+this._gridWidth()).attr("y2",y).style("stroke",this.state.targetGroupDividerLine.color).style("stroke-width",this.state.targetGroupDividerLine.thickness).style("shape-rendering","crispEdges")}else{var x=gridRegion.x+gridRegion.cellPad*columnsCounter[i-1]-(gridRegion.cellPad-gridRegion.cellSize)/2;this.state.svg.append("line").attr("class","pg_target_grp_divider").attr("x1",x).attr("y1",gridRegion.y-gridRegion.colLabelOffset).attr("x2",x).attr("y2",gridRegion.y+this._gridHeight()).style("stroke",this.state.targetGroupDividerLine.color).style("stroke-width",this.state.targetGroupDividerLine.thickness).style("shape-rendering","crispEdges");this.state.svg.append("line").attr("class","pg_target_grp_divider").attr("transform","rotate(-45 "+x+" "+(gridRegion.y-gridRegion.colLabelOffset)+")").attr("x1",x).attr("y1",gridRegion.y-gridRegion.colLabelOffset).attr("x2",x+this.state.targetGroupDividerLine.rotatedDividerLength).attr("y2",gridRegion.y-gridRegion.colLabelOffset).style("stroke",this.state.targetGroupDividerLine.color).style("stroke-width",this.state.targetGroupDividerLine.thickness).style("shape-rendering","crispEdges")}}}},_createGrid:function(){var self=this;var xvalues=this.state.xAxisRender.entries();var yvalues=this.state.yAxisRender.entries();var gridRegion=this.state.gridRegion;var xScale=this.state.xAxisRender.getScale();var yScale=this.state.yAxisRender.getScale();if(this.state.owlSimFunction==='compare'){var matrix=this.state.dataManager.buildMatrix(xvalues,yvalues,!1,this.state.owlSimFunction)}else{var matrix=this.state.dataManager.buildMatrix(xvalues,yvalues,!1)}
var column=this.state.svg.selectAll(".column").data(xvalues).enter().append("g").attr("class",'column').style("font-size",'11px').attr("id",function(d,i){return self.state.pgInstanceId+"_grid_col_"+i}).attr("transform",function(d,i){var offset=gridRegion.colLabelOffset;return"translate("+(gridRegion.x+(i*gridRegion.cellPad))+","+(gridRegion.y-offset)+")rotate(-45)"});column.append("text").attr("x",0).attr("y",xScale.rangeBand()+2).attr("dy",".32em").style('fill',function(d){if(d.type==='genotype'&&typeof(d.parentGeneID)!=='undefined'){return'#EA763B'}else{return''}}).attr("data-tooltip",this.state.pgInstanceId+"_tooltip").attr("text-anchor","start").text(function(d){return Utils.getShortLabel(d.label,self.state.targetLabelCharLimit)}).on("mouseover",function(d){self._mouseover(this,d,self)}).on("mouseout",function(d){self._mouseout()});if(this.state.selectedCompareTargetGroup.length===1&&this.state.selectedCompareTargetGroup[0].groupName!=='compare'){column.append("rect").attr("y",xScale.rangeBand()-1+gridRegion.colLabelOffset).attr('width',gridRegion.cellSize).attr('height',self._gridHeight()).style('fill',function(d){if(d.type==='genotype'&&typeof(d.parentGeneID)!=='undefined'){return'#ededed'}else{return'none'}}).style('opacity',0.8).attr("transform",function(d){return"rotate(45)"})}
self._createTextScores();var row=this.state.svg.selectAll(".row").data(matrix).enter().append("g").attr("class","row").attr("id",function(d,i){return self.state.pgInstanceId+"_grid_row_"+i}).attr("transform",function(d,i){var y=self.state.gridRegion.y;var ypad=self.state.gridRegion.cellPad;return"translate("+gridRegion.x+","+(y+(i*ypad))+")"});row.append("text").attr("x",-gridRegion.rowLabelOffset).attr("y",function(d,i){var rb=yScale.rangeBand(i)/2;return rb}).attr("dy",".80em").attr("text-anchor","end").style("font-size","11px").style('fill',function(d,i){var el=self.state.yAxisRender.itemAt(i);if(el.type==='genotype'&&typeof(el.parentGeneID)!=='undefined'){return'#EA763B'}else{return''}}).attr("data-tooltip",this.state.pgInstanceId+"_tooltip").text(function(d,i){var el=self.state.yAxisRender.itemAt(i);return Utils.getShortLabel(el.label)}).on("mouseover",function(d,i){var data=self.state.yAxisRender.itemAt(i);self._mouseover(this,data,self)}).on("mouseout",function(){self._mouseout()});if(this.state.selectedCompareTargetGroup.length===1&&this.state.selectedCompareTargetGroup[0].groupName!=='compare'){row.append("rect").attr('width',self._gridWidth()).attr('height',gridRegion.cellSize).style('fill',function(d,i){var el=self.state.yAxisRender.itemAt(i);if(el.type==='genotype'&&typeof(el.parentGeneID)!=='undefined'){return'#ededed'}else{return'none'}}).style('opacity',0.8)}
row.each(createrow);function createrow(row){var cell=d3.select(this).selectAll(".cell").data(row).enter().append("rect").attr("id",function(d){return self.state.pgInstanceId+"_cell_"+d.ypos+"_"+d.xpos}).attr("class","cell").attr("x",function(d){return d.xpos*gridRegion.cellPad}).attr("width",gridRegion.cellSize).attr("height",gridRegion.cellSize).attr("data-tooltip","tooltip").style("fill",function(d){var el=self.state.dataManager.getCellDetail(d.source_id,d.target_id,d.targetGroup);return self._getCellColor(el.value[self.state.selectedCalculation])}).on("mouseover",function(d){self._mouseover(this,d,self)}).on("mouseout",function(){self._mouseout()})}},_updateGrid:function(newXPos,newYPos){var xSize=this.state.xAxisRender.groupLength();var ySize=this.state.yAxisRender.groupLength();this.state.currXIdx=(newXPos>=xSize)?xSize:newXPos;this.state.currYIdx=(newYPos>=ySize)?ySize:newYPos;this.state.xAxisRender.setRenderStartPos(this.state.currXIdx-this.state.xAxisRender.displayLength());this.state.xAxisRender.setRenderEndPos(this.state.currXIdx);this.state.yAxisRender.setRenderStartPos(this.state.currYIdx-this.state.yAxisRender.displayLength());this.state.yAxisRender.setRenderEndPos(this.state.currYIdx);this._recreateGrid()},_updateVerticalGrid:function(newYPos){var ySize=this.state.yAxisRender.groupLength();this.state.currYIdx=(newYPos>=ySize)?ySize:newYPos;this.state.yAxisRender.setRenderStartPos(this.state.currYIdx-this.state.yAxisRender.displayLength());this.state.yAxisRender.setRenderEndPos(this.state.currYIdx);this._recreateGrid()},_updateHorizontalGrid:function(newXPos){var xSize=this.state.xAxisRender.groupLength();this.state.currXIdx=(newXPos>=xSize)?xSize:newXPos;this.state.xAxisRender.setRenderStartPos(this.state.currXIdx-this.state.xAxisRender.displayLength());this.state.xAxisRender.setRenderEndPos(this.state.currXIdx);this._recreateGrid()},_recreateGrid:function(){this._clearGrid();this._createGrid();this._relinkTooltip()},_clearGrid:function(){this.state.svg.selectAll("g.row").remove();this.state.svg.selectAll("g.column").remove();this.state.svg.selectAll("g.pg_score_text").remove()},_populateDialog:function(text){var SplitText="Title";var $dialog=$('<div></div>').html(SplitText).dialog({modal:!0,width:400,minHeight:200,height:260,maxHeight:300,minWidth:400,resizable:!1,draggable:!0,dialogClass:"pg_faq_dialog_bg_color",position:{my:"top",at:"top+25%",of:'#'+this.state.pgContainerId},title:'Phenogrid Notes'});$dialog.html(text);$dialog.dialog('open')},_createGradientLegend:function(){var gridRegion=this.state.gridRegion;if(this._isCrossComparisonView()){var x=gridRegion.x+this.state.multiTargetsModeTargetLengthLimit*this.state.selectedCompareTargetGroup.length*gridRegion.cellPad/2}else{var x=gridRegion.x+this.state.singleTargetModeTargetLengthLimit*gridRegion.cellPad/2}
var gradientGrp=this.state.svg.append("g").attr('id',this.state.pgInstanceId+'_gradient_legend');var gradient=gradientGrp.append("svg:defs").append("svg:linearGradient").attr("id",this.state.pgInstanceId+"_gradient_legend_fill").attr("x1","0").attr("x2","100%").attr("y1","0%").attr("y2","0%");for(var j in this.state.colorDomains){if(!this.state.colorDomains.hasOwnProperty(j)){break}
gradient.append("svg:stop").attr("offset",this.state.colorDomains[j]).style("stop-color",this.state.colorRanges[j])}
gradientGrp.append("rect").attr("x",x-this.state.gradientRegion.width/2).attr("y",gridRegion.y+this._gridHeight()+60).attr("id",this.state.pgInstanceId+"_gradient_legend_rect").attr("width",this.state.gradientRegion.width).attr("height",this.state.gradientRegion.height).attr("fill","url(#"+this.state.pgInstanceId+"_gradient_legend_fill)");var lowText,highText,labelText;for(var idx in this.state.similarityCalculation){if(!this.state.similarityCalculation.hasOwnProperty(idx)){break}
if(this.state.similarityCalculation[idx].calc===this.state.selectedCalculation){lowText=this.state.similarityCalculation[idx].low;highText=this.state.similarityCalculation[idx].high;labelText=this.state.similarityCalculation[idx].label;break}}
var gradientTextGrp=this.state.svg.select('#'+this.state.pgInstanceId+'_gradient_legend').append("g").attr('id',this.state.pgInstanceId+'_gradient_legend_texts').style('font-size','11px');var yTexts=gridRegion.y+this._gridHeight()+57;gradientTextGrp.append("svg:text").attr("x",x-this.state.gradientRegion.width/2).attr("y",yTexts).style('text-anchor','start').text(lowText);gradientTextGrp.append("svg:text").attr("x",x).attr("y",yTexts).style('text-anchor','middle').text(labelText);gradientTextGrp.append("svg:text").attr("x",x+this.state.gradientRegion.width/2).attr("y",yTexts).style('text-anchor','end').text(highText)},_createUnmatchedSources:function(){this.state.unmatchedSources=this._getUnmatchedSources();if(this.state.unmatchedSources.length>0){var pg_unmatched=$('<div id="'+this.state.pgInstanceId+'_unmatched" class="pg_unmatched"></div>');this.state.pgContainer.append(pg_unmatched);var pg_unmatched_list='<div id="'+this.state.pgInstanceId+'_unmatched_list" class="pg_unmatched_list"><i id="'+this.state.pgInstanceId+'_unmatched_close" class="fa fa-times pg_unmatched_close"></i><span class="pg_unmatched_list_arrow_border"></span><span class="pg_unmatched_list_arrow"></span><div id="'+this.state.pgInstanceId+'_unmatched_list_data"></div></div>';var pg_unmatched_btn='<div id="'+this.state.pgInstanceId+'_unmatched_btn" class="pg_unmatched_btn"><i class="fa fa-exclamation-triangle"></i> '+this.state.unmatchedButtonLabel+' </div>';pg_unmatched.append(pg_unmatched_list);pg_unmatched.append(pg_unmatched_btn);$('#'+this.state.pgInstanceId+'_unmatched_list').hide();if(this.state.gridSkeletonDataVendor==='IMPC'){var vendorDataUnmatchedSources=[];for(var i=0;i<this.state.unmatchedSources.length;i++){for(var idx in this.state.gridSkeletonData.yAxis){if(this.state.gridSkeletonData.yAxis[idx].id===this.state.unmatchedSources[i]){var item={id:this.state.gridSkeletonData.yAxis[idx].id,label:this.state.gridSkeletonData.yAxis[idx].term};vendorDataUnmatchedSources.push(item);break}}}
for(var j=0;j<vendorDataUnmatchedSources.length;j++){var pg_unmatched_list_item='<div class="pg_unmatched_list_item"><a href="'+this.state.serverURL+'/phenotype/'+vendorDataUnmatchedSources[j].id+'" target="_blank">'+vendorDataUnmatchedSources[j].label+'</a></div>';$('#'+this.state.pgInstanceId+'_unmatched_list_data').append(pg_unmatched_list_item)}}else{this._formatUnmatchedSources(this.state.unmatchedSources)}
this._positionUnmatchedSources();this._toggleUnmatchedSources()}},_createPhenogridControls:function(){var self=this;var phenogridControls=$('<div id="'+this.state.pgInstanceId+'_controls" class="pg_controls"></div>');this.state.pgContainer.append(phenogridControls);var optionhtml='<div id="'+this.state.pgInstanceId+'_controls_options" class="pg_controls_options"><i id="'+this.state.pgInstanceId+'_controls_close" class="fa fa-times pg_controls_close"></i><span class="pg_controls_options_arrow_border"></span><span class="pg_controls_options_arrow"></span></div>';var slideBtn='<div id="'+this.state.pgInstanceId+'_slide_btn" class="pg_slide_btn"><i class="fa fa-bars"></i> '+this.state.optionsBtnText+'</div>';var options=$(optionhtml);if(this.state.initialTargetGroupLoadList.length>1){var targetGroupSelection=this._createTargetGroupSelection();options.append(targetGroupSelection)}
var sortSel=this._createSortPhenotypeSelection();options.append(sortSel);var calcSel=this._createCalculationSelection();options.append(calcSel);var axisSel=this._createAxisSelection();options.append(axisSel);var exportBtn=this._createExportPhenogridButton();options.append(exportBtn);var aboutPhenogrid=this._createAboutPhenogrid();options.append(aboutPhenogrid);phenogridControls.append(options);phenogridControls.append(slideBtn);$('#'+this.state.pgInstanceId+'_controls_options').hide();$('#'+this.state.pgInstanceId+'_targetGroup').change(function(d){var items=this.childNodes;var temp=[];for(var idx=0;idx<items.length;idx++){if(items[idx].childNodes[0].checked){var rec=self._getTargetGroupInfo(self,items[idx].textContent);temp.push(rec)}}
if(temp.length>0){self.state.selectedCompareTargetGroup=temp}else{alert("You must have at least 1 target group selected.")}
self._createAxisRenderingGroups();$('#'+self.state.pgInstanceId+'_unmatched').remove();self._createUnmatchedSources();self._updateDisplay()});$('#'+this.state.pgInstanceId+'_calculation').change(function(d){self.state.selectedCalculation=parseInt(d.target.value);self._updateDisplay()});$('#'+this.state.pgInstanceId+'_sortphenotypes').change(function(d){self.state.selectedSort=d.target.value;if(self.state.invertAxis){self.state.xAxisRender.sort(self.state.selectedSort)}else{self.state.yAxisRender.sort(self.state.selectedSort)}
self._updateDisplay()});$("#"+this.state.pgInstanceId+"_invert_axis").click(function(){var $this=$(this);if($this.is(':checked')){self.state.invertAxis=!0}else{self.state.invertAxis=!1}
self._setAxisRenderers();self._updateDisplay()});$("#"+this.state.pgInstanceId+"_export").click(function(){var svgElementClone=$('#'+self.state.pgInstanceId+'_svg').clone();svgElementClone.find('#'+self.state.pgInstanceId+'_logo').attr('href',images.logo);svgElementClone.find('#'+self.state.pgInstanceId+'_scores_tip_icon').remove();svgElementClone.find('#'+self.state.pgInstanceId+'_monarchinitiative_text').removeClass('pg_hide');var svgStr='<svg xmlns="http://www.w3.org/2000/svg">'+svgElementClone.html()+'</svg>';var blob=new Blob([svgStr],{type:"image/svg+xml"});filesaver.saveAs(blob,"phenogrid.svg")});$('#'+this.state.pgInstanceId+'_sorts_faq').click("click",function(){self._populateDialog(htmlnotes.sorts)});$('#'+this.state.pgInstanceId+'_calcs_faq').click(function(){self._populateDialog(htmlnotes.calcs)});$('#'+this.state.pgInstanceId+'_about_phenogrid').click(function(){self._populateDialog(htmlnotes.faq)})},_createMonarchInitiativeRecognition:function(){var gridRegion=this.state.gridRegion;if(this._isCrossComparisonView()){var x=gridRegion.x+this.state.multiTargetsModeTargetLengthLimit*this.state.selectedCompareTargetGroup.length*gridRegion.cellPad/2}else{var x=gridRegion.x+this.state.singleTargetModeTargetLengthLimit*gridRegion.cellPad/2}
var recognitionGrp=this.state.svg.append("g").attr('id',this.state.pgInstanceId+'_recognition');recognitionGrp.append("text").attr("x",x).attr("y",this.state.gridRegion.y+this._gridHeight()+90).attr("id",this.state.pgInstanceId+"_monarchinitiative_text").style('font-size','10px').text(this.state.monarchInitiativeText);var self=this;recognitionGrp.append("svg:image").attr("xlink:href",images.logo).attr("x",x+$("#"+this.state.pgInstanceId+"_monarchinitiative_text")[0].getBoundingClientRect().width+3).attr("y",this.state.gridRegion.y+this._gridHeight()+74).attr("id",this.state.pgInstanceId+"_logo").attr('class','pg_cursor_pointer').attr("width",40).attr("height",26).on('click',function(){window.open(self.state.serverURL,'_blank')});var recognitionGrpWidth=$("#"+this.state.pgInstanceId+"_recognition")[0].getBoundingClientRect().width;recognitionGrp.attr("transform","translate("+ -recognitionGrpWidth/2+"0)")},_positionPhenogridControls:function(){var gridRegion=this.state.gridRegion;var marginTop=17;$('#'+this.state.pgInstanceId+'_slide_btn').css('top',gridRegion.y+this._gridHeight()+marginTop);var pg_ctrl_options=$('#'+this.state.pgInstanceId+'_controls_options');if(this.state.initialTargetGroupLoadList.length===1){pg_ctrl_options.css('height',310)}
pg_ctrl_options.css('top',gridRegion.y+this._gridHeight()-pg_ctrl_options.outerHeight()-10+marginTop);if(this._isCrossComparisonView()){pg_ctrl_options.css('left',gridRegion.x+this.state.multiTargetsModeTargetLengthLimit*this.state.selectedCompareTargetGroup.length*gridRegion.cellPad+gridRegion.rowLabelOffset);$('#'+this.state.pgInstanceId+'_slide_btn').css('left',gridRegion.x+this.state.multiTargetsModeTargetLengthLimit*this.state.selectedCompareTargetGroup.length*gridRegion.cellPad+gridRegion.rowLabelOffset)}else{pg_ctrl_options.css('left',gridRegion.x+this.state.singleTargetModeTargetLengthLimit*gridRegion.cellPad+gridRegion.rowLabelOffset);$('#'+this.state.pgInstanceId+'_slide_btn').css('left',gridRegion.x+this.state.singleTargetModeTargetLengthLimit*gridRegion.cellPad+gridRegion.rowLabelOffset)}},_createTargetGroupSelection:function(){var optionhtml="<div class='pg_ctrl_label'>Target Group(s)</div>"+"<div id='"+this.state.pgInstanceId+"_targetGroup'>";for(var idx in this.state.targetGroupList){if(!this.state.targetGroupList.hasOwnProperty(idx)){break}
var checked="";var disabled="";var linethrough="";if(this._isTargetGroupSelected(this,this.state.targetGroupList[idx].groupName)){checked="checked"}
if(this.state.dataManager.length('target',this.state.targetGroupList[idx].groupName)===0){disabled="disabled";linethrough="pg_linethrough"}
optionhtml+="<div class='pg_select_item "+linethrough+"'><input type='checkbox' value=\""+this.state.targetGroupList[idx].groupName+"\" "+checked+disabled+">"+this.state.targetGroupList[idx].groupName+'</div>'}
optionhtml+="</div><div class='pg_hr'></div>";return $(optionhtml)},_createCalculationSelection:function(){var optionhtml="<div class='pg_ctrl_label'>Calculation Method"+" <i class='fa fa-info-circle cursor_pointer' id='"+this.state.pgInstanceId+"_calcs_faq'></i></div>"+"<div id='"+this.state.pgInstanceId+"_calculation'>";for(var idx in this.state.similarityCalculation){if(!this.state.similarityCalculation.hasOwnProperty(idx)){break}
var checked="";if(this.state.similarityCalculation[idx].calc===this.state.selectedCalculation){checked="checked"}
...
optionhtml+="</div><div class='pg_hr'></div>";return $(optionhtml)},_createSortPhenotypeSelection:function(){var optionhtml="<div class='pg_ctrl_label'>Sort Phenotypes"+' <i class="fa fa-info-circle cursor_pointer" id="'+this.state.pgInstanceId+'_sorts_faq"></i></div>'+'<div id="'+this.state.pgInstanceId+'_sortphenotypes">';for(var idx in this.state.phenotypeSort){if(!this.state.phenotypeSort.hasOwnProperty(idx)){break}
var checked="";if(this.state.phenotypeSort[idx]===this.state.selectedSort){checked="checked"}
...
optionhtml+="</div><div class='pg_hr'></div>";return $(optionhtml)},_createAxisSelection:function(){var checked="";if(this.state.invertAxis){checked="checked"}
var optionhtml='<div class="pg_select_item"><input type="checkbox" id="'+this.state.pgInstanceId+'_invert_axis"'+checked+'>Invert Axis</div><div class="pg_hr"></div>';return $(optionhtml)},_createAboutPhenogrid:function(){var html='<div class="pg_select_item">About Phenogrid <i class="fa fa-info-circle cursor_pointer" id="'+this.state.pgInstanceId+'_about_phenogrid"></i></div>';return $(html)},_createExportPhenogridButton:function(){var btn='<div id="'+this.state.pgInstanceId+'_export" class="pg_export">Save as SVG...</div><div class="pg_hr"></div>';return $(btn)},_getUnmatchedSources:function(){var origSourceList=this.state.dataLoader.origSourceList;var matchedList=this.state.yAxisRender.groupIDs();var normalizedMatchedList=[];var unmatchedList=[];for(var j in matchedList){normalizedMatchedList.push(matchedList[j].replace("_",":"))}
for(var i in origSourceList){if(normalizedMatchedList.indexOf(origSourceList[i])===-1){unmatchedList.push(origSourceList[i])}}
return unmatchedList},_positionUnmatchedSources:function(){var gridRegion=this.state.gridRegion;$('#'+this.state.pgInstanceId+'_unmatched_btn').css('top',gridRegion.y+this._gridHeight()+17);$('#'+this.state.pgInstanceId+'_unmatched_btn').css('left',this.state.btnPadding);$('#'+this.state.pgInstanceId+'_unmatched_list').css('top',gridRegion.y+this._gridHeight()+$('#'+this.state.pgInstanceId+'_unmatched_btn').outerHeight()+ +17+10);if(this._isCrossComparisonView()){$('#'+this.state.pgInstanceId+'_unmatched_list').css('width',gridRegion.x+this.state.multiTargetsModeTargetLengthLimit*this.state.selectedCompareTargetGroup.length*gridRegion.cellPad-20)}else{$('#'+this.state.pgInstanceId+'_unmatched_list').css('width',gridRegion.x+this.state.singleTargetModeTargetLengthLimit*gridRegion.cellPad-20)}},_fetchSourceLabelCallback:function(self,target,targets,data){var label;if(typeof(data.label)!=='undefined'){label=Utils.getShortLabel(data.label,self.state.targetLabelCharLimit)}else{label=data.id}
var pg_unmatched_list_item='<div class="pg_unmatched_list_item"><a href="'+self.state.serverURL+'/phenotype/'+data.id+'" target="_blank">'+label+'</a></div>';$('#'+self.state.pgInstanceId+'_unmatched_list_data').append(pg_unmatched_list_item);self._formatUnmatchedSources(targets)},_fetchUnmatchedLabel:function(target,targets,callback){var self=this;var jqxhr=$.ajax({url:this.state.serverURL+"/phenotype/"+target+".json",async:!0,method:'GET',dataType:'json'});jqxhr.done(function(data){callback(self,target,targets,data)});jqxhr.fail(function(){console.log('Ajax error - _fetchUnmatchedLabel()')})},_formatUnmatchedSources:function(targetGrpList){if(targetGrpList.length>0){var target=targetGrpList[0];targetGrpList=targetGrpList.slice(1);var callback=this._fetchSourceLabelCallback;this._fetchUnmatchedLabel(target,targetGrpList,callback)}},_removeDuplicatedSourceId:function(gridSourceList){var filteredList={};var newlist=[];var pheno;for(var i in gridSourceList){pheno=gridSourceList[i];if(typeof pheno==='string'){newlist.push(pheno)}}
newlist=newlist.filter(function(item){return filteredList.hasOwnProperty(item)?!1:(filteredList[item]=!0)});return newlist},_expandOntology:function(id){var cache=this.state.dataLoader.checkOntologyCache(id);if(typeof(cache)==='undefined'){var cb=this._postExpandOntologyCB;this.state.dataLoader.getOntology(id,this.state.ontologyDirection,this.state.ontologyDepth,cb,this)}else{this._postExpandOntologyCB(cache,id,this)}},_postExpandOntologyCB:function(d,id,parent){parent.state.ontologyTreesDone=0;parent.state.ontologyTreeHeight=0;var info=parent._getAxisData(id);var hrefLink='<a href="'+parent.state.serverURL+'/phenotype/'+id+'" target="_blank">'+info.label+'</a>';var ontologyData="<strong>Phenotype: </strong> "+hrefLink+"<br>";ontologyData+="<strong>IC:</strong> "+info.IC.toFixed(2)+"<br>";ontologyData+="<strong>Sum:</strong> "+info.sum.toFixed(2)+"<br>";ontologyData+="<strong>Frequency:</strong> "+info.count+"<br><br>";var classTree=parent._buildOntologyTree(id.replace("_",":"),d.edges,0);if(classTree==="<br>"){ontologyData+="<em>No classification hierarchy data found</em>"}else{ontologyData+="<strong>Classification hierarchy:</strong>"+classTree}
$('#'+parent.state.pgInstanceId+'_tooltip_inner').html(ontologyData)},_insertExpandedItems:function(id){$('.pg_expand_genotype_icon').removeClass('fa-plus-circle');$('.pg_expand_genotype_icon').addClass('fa-spinner fa-pulse');var group_name=this.state.selectedCompareTargetGroup[0].groupName;var loaded=this.state.dataManager.checkExpandedItemsLoaded(group_name,id);if(loaded){var associated_genotype_ids=this.state.dataLoader.loadedNewTargetGroupItems[id];for(var i=0;i<associated_genotype_ids.length;i++){var genotype_id=associated_genotype_ids[i];this.state.dataLoader.targetData[group_name][genotype_id].visible=!0}
this.state.reactivateTargetGroupItems[group_name]=!0;this._updateTargetAxisRenderingGroup(group_name);this.state.reactivateTargetGroupItems[group_name]=!1;this._updateDisplay();$('.pg_expand_genotype_icon').removeClass('fa-spinner fa-pulse');$('.pg_expand_genotype_icon').addClass('fa-plus-circle');this.state.dataManager.expandedItemList[id]=this.state.dataLoader.loadedNewTargetGroupItems[id]}else{var cb=this._insertExpandedItemsCb;this.state.dataLoader.getNewTargetGroupItems(id,cb,this)}},_insertExpandedItemsCb:function(results,id,parent,errorMsg){console.log(results);if(typeof(errorMsg)==='undefined'){if(results.b.length>0){var group_name=parent.state.selectedCompareTargetGroup[0].groupName;parent.state.dataLoader.transformNewTargetGroupItems(group_name,results,id);parent._updateTargetAxisRenderingGroup(group_name);if(typeof(parent.state.dataManager.reorderedTargetEntriesIndexArray[group_name])==='undefined'){parent.state.dataManager.reorderedTargetEntriesIndexArray[group_name]=[]}
if(parent.state.dataManager.reorderedTargetEntriesIndexArray[group_name].length===0){var updatedTargetEntries=parent.state.targetAxis.groupEntries()}else{var updatedTargetEntries=parent.state.dataManager.appendNewItemsToOrderedTargetList(group_name,results.b)}
var newItemsData={targetEntries:updatedTargetEntries,genotypes:results.b,parentGeneID:id,group:group_name};parent.state.dataManager.updateTargetList(newItemsData);parent.state.newTargetGroupItems[group_name]=!0;parent._updateTargetAxisRenderingGroup(group_name);parent.state.newTargetGroupItems[group_name]=!1;parent.state.expandedTargetGroupItems[group_name]=!0;parent._updateDisplay();$('.pg_expand_genotype_icon').removeClass('fa-spinner fa-pulse');$('.pg_expand_genotype_icon').addClass('fa-plus-circle');parent.state.dataManager.expandedItemList[id]=parent.state.dataLoader.loadedNewTargetGroupItems[id]}}else{parent._populateDialog(errorMsg)}},_removeExpandedItems:function(id){var group_name=this.state.selectedCompareTargetGroup[0].groupName;var associated_genotype_ids=this.state.dataLoader.loadedNewTargetGroupItems[id];for(var i=0;i<associated_genotype_ids.length;i++){var genotype_id=associated_genotype_ids[i];this.state.dataLoader.targetData[group_name][genotype_id].visible=!1}
delete this.state.dataManager.expandedItemList[id];this.state.removedTargetGroupItems[group_name]=!0;this._updateTargetAxisRenderingGroup(group_name);this.state.removedTargetGroupItems[group_name]=!1;this._updateDisplay()},_isTargetGroupSelected:function(self,name){for(var i in self.state.selectedCompareTargetGroup){if(self.state.selectedCompareTargetGroup[i].groupName===name){return!0}}
return!1},_getTargetGroupInfo:function(self,name){for(var i in self.state.targetGroupList){if(self.state.targetGroupList[i].groupName===name){return self.state.targetGroupList[i]}}},_isCrossComparisonView:function(){if(this.state.selectedCompareTargetGroup.length===1){return!1}
return!0}})})}())},{"./axisgroup.js":1,"./dataloader.js":2,"./datamanager.js":3,"./htmlnotes.json":4,"./images.json":5,"./utils.js":7,"d3":8,"filesaver.js":9,"jquery":11,"jquery-ui":10}],7:[function(require,module,exports){(function(){'use strict';var $=require('jquery');var Utils={getConceptId:function(uri){var retString=uri;try{retString=retString.replace(" ","_");retString=retString.replace(":","_");return retString}catch(exception){}},normalizeIC:function(datarow,maxICScore){var aIC=datarow.a.IC;var bIC=datarow.b.IC;var lIC=datarow.lcs.IC;var nic;var ics=new Array(3);nic=Math.sqrt((Math.pow(aIC-lIC,2))+(Math.pow(bIC-lIC,2)));nic=(1-(nic/+maxICScore))*100;ics[0]=nic;nic=((lIC/aIC)*100);ics[1]=nic;nic=lIC;ics[2]=nic;nic=((lIC/bIC)*100);ics[3]=nic;return ics},getShortLabel:function(label,newlength){if(label!==undefined){var retLabel=label;if(!newlength){newlength=30}
if(label.length>newlength){retLabel=label.substring(0,newlength-3)+"..."}
return retLabel}else{return"Unknown"}},encodeHtmlEntity:function(str){if(str!==null){return str.replace(/»/g,"&#187;").replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;").replace(/"/g,"&quot;").replace(/'/g,"&#039;")}
return str},decodeHtmlEntity:function(str){return $('<div></div>').html(str).text()},formatScore:function(score){if(score===0){return""}else{return"(IC: "+score+")"}},capitalizeString:function(word){if(word===undefined){return"Undefined"}else if(word===null){return""}else{return word.charAt(0).toUpperCase()+word.slice(1)}},toProperCase:function(oldstring){return oldstring.replace(/\w\S*/g,function(txt){return txt.charAt(0).toUpperCase()+txt.substr(1).toLowerCase()})}};module.exports=Utils}())},{"jquery":11}],8:[function(require,module,exports){!function(){var d3={version:"3.5.16"};var d3_arraySlice=[].slice,d3_array=function(list){return d3_arraySlice.call(list)};var d3_document=this.document;function d3_documentElement(node){return node&&(node.ownerDocument||node.document||node).documentElement}
...
function d3_class(ctor,properties){for(var key in properties){Object.defineProperty(ctor.prototype,key,{value:properties[key],enumerable:!1})}}
...
function d3_map_empty(){for(var key in this._)return!1;return!0}
...
d3_class(d3_Set,{has:d3_map_has,add:function(key){this._[d3_map_escape(key+="")]=!0;return key},remove:d3_map_remove,values:d3_map_keys,size:d3_map_size,empty:d3_map_empty,forEach:function(f){for(var key in this._)f.call(this,d3_map_unescape(key));}});d3.behavior={};function d3_identity(d){return d}
...
for(i=-1;++i<m;){if(!(node=nodeByKeyValue.get(keyValue=key.call(groupData,nodeData=groupData[i],i)))){enterNodes[i]=d3_selection_dataNode(nodeData)}else if(node!==!0){updateNodes[i]=node;node.__data__=nodeData}
nodeByKeyValue.set(keyValue,!0)}
for(i=-1;++i<n;){if(i in keyValues&&nodeByKeyValue.get(keyValues[i])!==!0){exitNodes[i]=group[i]}}}else{for(i=-1;++i<n0;){node=group[i];nodeData=groupData[i];if(node){node.__data__=nodeData;updateNodes[i]=node}else{enterNodes[i]=d3_selection_dataNode(nodeData)}}
...
return d3_selection([group])};d3_selectionPrototype.on=function(type,listener,capture){var n=arguments.length;if(n<3){if(typeof type!=="string"){if(n<2)listener=!1;for(capture in type)this.each(d3_selection_on(capture,type[capture],listener));return this}
...
var d3_event_dragSelect,d3_event_dragId=0;function d3_event_dragSuppress(node){var name=".dragsuppress-"+ ++d3_event_dragId,click="click"+name,w=d3.select(d3_window(node)).on("touchmove"+name,d3_eventPreventDefault).on("dragstart"+name,d3_eventPreventDefault).on("selectstart"+name,d3_eventPreventDefault);if(d3_event_dragSelect==null){d3_event_dragSelect="onselectstart" in node?!1:d3_vendorSymbol(node.style,"userSelect")}
...
return function(suppressClick){w.on(name,null);if(d3_event_dragSelect)style[d3_event_dragSelect]=select;if(suppressClick){var off=function(){w.on(click,null)};w.on(click,function(){d3_eventPreventDefault();off()},!0);setTimeout(off,0)}}}
...
request.onprogress=function(event){var o=d3.event;d3.event=event;try{dispatch.progress.call(xhr,request)}finally{d3.event=o}};xhr.header=function(name,value){name=(name+"").toLowerCase();if(arguments.length<2)return headers[name];if(value==null)delete headers[name];else headers[name]=value+"";return xhr};xhr.mimeType=function(value){if(!arguments.length)return mimeType;mimeType=value==null?null:value+"";return xhr};xhr.responseType=function(value){if(!arguments.length)return responseType;responseType=value;return xhr};xhr.response=function(value){response=value;return xhr};["get","post"].forEach(function(method){xhr[method]=function(){return xhr.send.apply(xhr,[method].concat(d3_array(arguments)))}});xhr.send=function(method,data,callback){if(arguments.length===2&&typeof data==="function")callback=data,data=null;request.open(method,url,!0);if(mimeType!=null&&!("accept" in headers))headers.accept=mimeType+",*/*";if(request.setRequestHeader)for(var name in headers)request.setRequestHeader(name,headers[name]);if(mimeType!=null&&request.overrideMimeType)request.overrideMimeType(mimeType);if(responseType!=null)request.responseType=responseType;if(callback!=null)xhr.on("error",callback).on("load",function(request){callback(null,request)});dispatch.beforesend.call(xhr,request);request.send(data==null?null:data);return xhr};xhr.abort=function(){request.abort();return xhr};d3.rebind(xhr,dispatch,"on");return callback==null?xhr:xhr.get(d3_xhr_fixCallback(callback))}
...
I=i+2;var c=text.charCodeAt(i+1);if(c===13){eol=!0;if(text.charCodeAt(i+2)===10)++I}else if(c===10){eol=!0}
return text.slice(j+1,i).replace(/""/g,'"')}
while(I<N){var c=text.charCodeAt(I++),k=1;if(c===10)eol=!0;else if(c===13){eol=!0;if(text.charCodeAt(I)===10)++I,++k}else if(c!==delimiterCode)continue;return text.slice(j,I-k)}
return text.slice(j)}
...
return t.reverse().join(locale_thousands)}:d3_identity;return function(specifier){var match=d3_format_re.exec(specifier),fill=match[1]||" ",align=match[2]||">",sign=match[3]||"-",symbol=match[4]||"",zfill=match[5],width=+match[6],comma=match[7],precision=match[8],type=match[9],scale=1,prefix="",suffix="",integer=!1,exponent=!0;if(precision)precision=+precision.substring(1);if(zfill||fill==="0"&&align==="="){zfill=fill="0";align="="}
switch(type){case "n":comma=!0;type="g";break;case "%":scale=100;suffix="%";type="f";break;case "p":scale=100;suffix="%";type="r";break;case "b":case "o":case "x":case "X":if(symbol==="#")prefix="0"+type.toLowerCase();case "c":exponent=!1;case "d":integer=!0;precision=0;break;case "s":scale=-1;type="r";break}
...
a.n=b=array[0];b.p=a}
function d3_geo_clipPolygonIntersection(point,points,other,entry){this.x=point;this.z=points;this.o=other;this.e=entry;this.v=!1;this.n=this.p=null}
function d3_geo_clip(pointVisible,clipLine,interpolate,clipStart){return function(rotate,listener){var line=clipLine(listener),rotatedClipStart=rotate.invert(clipStart[0],clipStart[1]);var clip={point:point,lineStart:lineStart,lineEnd:lineEnd,polygonStart:function(){clip.point=pointRing;clip.lineStart=ringStart;clip.lineEnd=ringEnd;segments=[];polygon=[]},polygonEnd:function(){clip.point=point;clip.lineStart=lineStart;clip.lineEnd=lineEnd;segments=d3.merge(segments);var clipStartInside=d3_geo_pointInPolygon(rotatedClipStart,polygon);if(segments.length){if(!polygonStarted)listener.polygonStart(),polygonStarted=!0;d3_geo_clipPolygon(segments,d3_geo_clipSort,clipStartInside,interpolate,listener)}else if(clipStartInside){if(!polygonStarted)listener.polygonStart(),polygonStarted=!0;listener.lineStart();interpolate(null,null,1,listener);listener.lineEnd()}
if(polygonStarted)listener.polygonEnd(),polygonStarted=!1;segments=polygon=null},sphere:function(){listener.polygonStart();listener.lineStart();interpolate(null,null,1,listener);listener.lineEnd();listener.polygonEnd()}};function point(λ,φ){var point=rotate(λ,φ);if(pointVisible(λ=point[0],φ=point[1]))listener.point(λ,φ)}
function pointLine(λ,φ){var point=rotate(λ,φ);line.point(point[0],point[1])}
function lineStart(){clip.point=pointLine;line.lineStart()}
function lineEnd(){clip.point=point;line.lineEnd()}
var segments;var buffer=d3_geo_clipBufferListener(),ringListener=clipLine(buffer),polygonStarted=!1,polygon,ring;function pointRing(λ,φ){ring.push([λ,φ]);var point=rotate(λ,φ);ringListener.point(point[0],point[1])}
function ringStart(){ringListener.lineStart();ring=[]}
function ringEnd(){pointRing(ring[0][0],ring[0][1]);ringListener.lineEnd();var clean=ringListener.clean(),ringSegments=buffer.buffer(),segment,n=ringSegments.length;ring.pop();polygon.push(ring);ring=null;if(!n)return;if(clean&1){segment=ringSegments[0];var n=segment.length-1,i=-1,point;if(n>0){if(!polygonStarted)listener.polygonStart(),polygonStarted=!0;listener.lineStart();while(++i<n)listener.point((point=segment[i])[0],point[1]);listener.lineEnd()}
return}
...
point0=point2}else if(notHemisphere&&point0&&smallRadius^v){var t;if(!(c&c0)&&(t=intersect(point1,point0,!0))){clean=0;if(smallRadius){listener.lineStart();listener.point(t[0][0],t[0][1]);listener.point(t[1][0],t[1][1]);listener.lineEnd()}else{listener.point(t[1][0],t[1][1]);listener.lineEnd();listener.lineStart();listener.point(t[0][0],t[0][1])}}}
...
var x__,y__,v__,x_,y_,v_,first,clean;function lineStart(){clip.point=linePoint;if(polygon)polygon.push(ring=[]);first=!0;v_=!1;x_=y_=NaN}
function lineEnd(){if(segments){linePoint(x__,y__);if(v__&&v_)bufferListener.rejoin();segments.push(bufferListener.buffer())}
clip.point=point;if(v_)listener.lineEnd()}
function linePoint(x,y){x=Math.max(-d3_geo_clipExtentMAX,Math.min(d3_geo_clipExtentMAX,x));y=Math.max(-d3_geo_clipExtentMAX,Math.min(d3_geo_clipExtentMAX,y));var v=pointVisible(x,y);if(polygon)ring.push([x,y]);if(first){x__=x,y__=y,v__=v;first=!1;if(v){listener.lineStart();listener.point(x,y)}}else{if(v&&v_)listener.point(x,y);else{var l={a:{x:x_,y:y_},b:{x:x,y:y}};if(clipLine(l)){if(!v_){listener.lineStart();listener.point(l.a.x,l.a.y)}
listener.point(l.b.x,l.b.y);if(!v)listener.lineEnd();clean=!1}else if(v){listener.lineStart();listener.point(x,y);clean=!1}}}
x_=x,y_=y,v_=v}
...
node.L=node.R=null;node.U=parent;node.C=!0;after=node;while(parent&&parent.C){grandpa=parent.U;if(parent===grandpa.L){uncle=grandpa.R;if(uncle&&uncle.C){parent.C=uncle.C=!1;grandpa.C=!0;after=grandpa}else{if(after===parent.R){d3_geom_voronoiRedBlackRotateLeft(this,parent);after=parent;parent=after.U}
parent.C=!1;grandpa.C=!0;d3_geom_voronoiRedBlackRotateRight(this,grandpa)}}else{uncle=grandpa.L;if(uncle&&uncle.C){parent.C=uncle.C=!1;grandpa.C=!0;after=grandpa}else{if(after===parent.L){d3_geom_voronoiRedBlackRotateRight(this,parent);after=parent;parent=after.U}
parent.C=!1;grandpa.C=!0;d3_geom_voronoiRedBlackRotateLeft(this,grandpa)}}
parent=after.U}
...
do{if(node===this._)break;if(node===parent.L){sibling=parent.R;if(sibling.C){sibling.C=!1;parent.C=!0;d3_geom_voronoiRedBlackRotateLeft(this,parent);sibling=parent.R}
if(sibling.L&&sibling.L.C||sibling.R&&sibling.R.C){if(!sibling.R||!sibling.R.C){sibling.L.C=!1;sibling.C=!0;d3_geom_voronoiRedBlackRotateRight(this,sibling);sibling=parent.R}
sibling.C=parent.C;parent.C=sibling.R.C=!1;d3_geom_voronoiRedBlackRotateLeft(this,parent);node=this._;break}}else{sibling=parent.L;if(sibling.C){sibling.C=!1;parent.C=!0;d3_geom_voronoiRedBlackRotateRight(this,parent);sibling=parent.L}
if(sibling.L&&sibling.L.C||sibling.R&&sibling.R.C){if(!sibling.L||!sibling.L.C){sibling.R.C=!1;sibling.C=!0;d3_geom_voronoiRedBlackRotateLeft(this,sibling);sibling=parent.L}
sibling.C=parent.C;parent.C=sibling.L.C=!1;d3_geom_voronoiRedBlackRotateRight(this,parent);node=this._;break}}
sibling.C=!0;node=parent;parent=parent.U}while(!node.C);if(node)node.C=!1}};function d3_geom_voronoiRedBlackRotateLeft(tree,node){var p=node,q=node.R,parent=p.U;if(parent){if(parent.L===p)parent.L=q;else parent.R=q}else{tree._=q}
...
function insertChild(n,d,x,y,x1,y1,x2,y2){var xm=(x1+x2)*.5,ym=(y1+y2)*.5,right=x>=xm,below=y>=ym,i=below<<1|right;n.leaf=!1;n=n.nodes[i]||(n.nodes[i]=d3_geom_quadtreeNode());if(right)x1=xm;else x2=xm;if(below)y1=ym;else y2=ym;insert(n,d,x,y,x1,y1,x2,y2)}
...
function d3_geom_quadtreeNode(){return{leaf:!0,nodes:[],point:null,x:null,y:null}}
...
return!quad.charge}}
force.tick=function(){if((alpha*=.99)<.005){timer=null;event.end({type:"end",alpha:alpha=0});return!0}
...
d3.layout.histogram=function(){var frequency=!0,valuer=Number,ranger=d3_layout_histogramRange,binner=d3_layout_histogramBinSturges;function histogram(data,i){var bins=[],values=data.map(valuer,this),range=ranger.call(this,values,i),thresholds=binner.call(this,range,values,i),bin,i=-1,n=values.length,m=thresholds.length-1,k=frequency?1:1/n,x;while(++i<m){bin=bins[i]=[];bin.dx=thresholds[i+1]-(bin.x=thresholds[i]);bin.y=0}
...
d3.layout.cluster=function(){var hierarchy=d3.layout.hierarchy().sort(null).value(null),separation=d3_layout_treeSeparation,size=[1,1],nodeSize=!1;function cluster(d,i){var nodes=hierarchy.call(this,d,i),root=nodes[0],previousNode,x=0;d3_layout_hierarchyVisitAfter(root,function(node){var children=node.children;if(children&&children.length){node.x=d3_layout_clusterX(children);node.y=d3_layout_clusterY(children)}else{node.x=previousNode?x+=separation(node,previousNode):0;node.y=0;previousNode=node}});var left=d3_layout_clusterLeft(root),right=d3_layout_clusterRight(root),x0=left.x-separation(left,right)/2,x1=right.x+separation(right,left)/2;d3_layout_hierarchyVisitAfter(root,nodeSize?function(node){node.x=(node.x-root.x)*size[0];node.y=(root.y-node.y)*size[1]}:function(node){node.x=(node.x-x0)/(x1-x0)*size[0];node.y=(1-(root.y?node.y/root.y:1))*size[1]});return nodes}
...
d3.layout.treemap=function(){var hierarchy=d3.layout.hierarchy(),round=Math.round,size=[1,1],padding=null,pad=d3_layout_treemapPadNull,sticky=!1,stickies,mode="squarify",ratio=.5*(1+Math.sqrt(5));function scale(children,k){var i=-1,n=children.length,child,area;while(++i<n){area=(child=children[i]).value*(k<0?0:k);child.area=isNaN(area)||area<=0?0:area}}
function squarify(node){var children=node.children;if(children&&children.length){var rect=pad(node),row=[],remaining=children.slice(),child,best=Infinity,score,u=mode==="slice"?rect.dx:mode==="dice"?rect.dy:mode==="slice-dice"?node.depth&1?rect.dy:rect.dx:Math.min(rect.dx,rect.dy),n;scale(remaining,rect.dx*rect.dy/node.value);row.area=0;while((n=remaining.length)>0){row.push(child=remaining[n-1]);row.area+=child.area;if(mode!=="squarify"||(score=worst(row,u))<=best){remaining.pop();best=score}else{row.area-=row.pop().area;position(row,u,rect,!1);u=Math.min(rect.dx,rect.dy);row.length=row.area=0;best=Infinity}}
...
d3.scale.linear=function(){return d3_scale_linear([0,1],[0,1],d3_interpolate,!1)};function d3_scale_linear(domain,range,interpolate,clamp){var output,input;function rescale(){var linear=Math.min(domain.length,range.length)>2?d3_scale_polylinear:d3_scale_bilinear,uninterpolate=clamp?d3_uninterpolateClamp:d3_uninterpolateNumber;output=linear(domain,range,uninterpolate,interpolate);input=linear(range,domain,uninterpolate,d3_interpolate);return scale}
...
d3.scale.log=function(){return d3_scale_log(d3.scale.linear().domain([0,1]),10,!0,[1,10])};function d3_scale_log(linear,base,positive,domain){function log(x){return(positive?Math.log(x<0?0:x):-Math.log(x>0?0:-x))/Math.log(base)}
...
function brushmove(){var point=d3.mouse(target),moved=!1;if(offset){point[0]+=offset[0];point[1]+=offset[1]}
...
if(extent[0]!=min||extent[1]!=max){if(i)yExtentDomain=null;else xExtentDomain=null;extent[0]=min;extent[1]=max;return!0}}
...
var filesaver=this,type=blob.type,blob_changed=!1,object_url,target_view,dispatch_all=function(){dispatch(filesaver,"writestart progress write writeend".split(" "))},fs_error=function(){if(blob_changed||!object_url){object_url=get_URL().createObjectURL(blob)}
...
if(can_use_save_link){object_url=get_URL().createObjectURL(blob);save_link.href=object_url;save_link.download=name;setTimeout(function(){click(save_link);dispatch_all();revoke(object_url);filesaver.readyState=filesaver.DONE});return}
