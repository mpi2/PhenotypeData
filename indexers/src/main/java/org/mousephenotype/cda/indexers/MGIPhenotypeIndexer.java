...
//            logger.info(" Creating temporary observations2 table took [took: {}s]", (System.currentTimeMillis() - tmpTableStartTime) / 1000.0);
    ...
    public int populateGenotypePhenotypeSolrCore(RunStatus runStatus) throws SQLException, IOException, SolrServerException {
         ...
         */
        String query="SELECT * FROM biological_model_phenotype bmp INNER JOIN biological_model bm ON bmp.biological_model_id = bm.id  INNER JOIN biological_model_allele bma ON bma.biological_model_id = bm.id INNER JOIN biological_model_genomic_feature bmgf ON bmgf.biological_model_id = bm.id "+ 
