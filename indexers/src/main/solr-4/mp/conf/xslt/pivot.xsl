...
<xsl:output method="text" encoding="iso-8859-1"/>
  ...
  <xsl:template match="lst[@name = 'facet_pivot']/arr/lst">
  	<xsl:variable name="v1" select="concat(concat('&quot;',(str|int|double|bool)[@name='value']), '&quot;,')"/>
  ...
  <xsl:template name="last">
  	...
  	<xsl:for-each select="arr/lst">
  		<xsl:if test="(str|int|double|bool)[@name='value']">
			<xsl:value-of select="$wholeLine"/>"<xsl:value-of select="(str|int|double|bool)[@name='value']"/>",<xsl:value-of select="normalize-space(int)"/><xsl:text>&#xD;</xsl:text>
		</xsl:if>
  	</xsl:for-each>
  </xsl:template>
  ...
  <xsl:template name="inner">
    <xsl:param name="line" />  
  	<xsl:for-each select="arr/lst">
		<xsl:variable name="currentLine" select="concat($line, concat(concat('&quot;',(str|int|double|bool)[@name='value']), '&quot;,'))"/>
		<xsl:if test="arr/lst/arr/lst">
  			<xsl:call-template name="inner">
