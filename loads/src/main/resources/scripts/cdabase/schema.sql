...
('March 7 KO (Cross 2)',NULL,'MGI:1931053',(SELECT @MGI_DB_ID),'MARCH7<not yet available>','C57BL/6J',(SELECT id FROM organisation WHERE name = 'MRC Harwell'),(SELECT @EUMODIC_PROJECT_ID),(SELECT id FROM organisation WHERE name = 'MRC Harwell'),(SELECT @EUMODIC_PROJECT_ID),(SELECT id FROM organisation WHERE name = 'MRC Harwell')),
...
-- NOTE THE TRAILING SPACE AFTER THE COLONY ID. IT COMES THAT WAY FROM THE DCC.
