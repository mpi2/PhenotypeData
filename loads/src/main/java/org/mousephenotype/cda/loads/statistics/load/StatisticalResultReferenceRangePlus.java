...
public class StatisticalResultReferenceRangePlus implements StatisticalResult, Serializable {
	...
	private String genderFemaleKoEstimate;
	private Double genderFemaleKoStandardErrorEstimate;
	private String genderFemaleKoPValue;
	private String genderMaleKoEstimate;
	private Double genderMaleKoStandardErrorEstimate;
	private String genderMaleKoPValue;
	...
	/**
	 * Return the pvalue for the specified sex (or overall genotype if sex is null) for the
	 * decreased or increased change in mean, if available.
	 *
	 * @param sex pvalue for specific sex, null for "no sex" a.k.a. genotype effect
	 * @param direction decreased vs increased (decreased = low vs normal+high, increased = high vs low+normal)
	 * @return
	 */
	public Float getPvalue(SexType sex, PhenotypeAnnotationType direction) {
		...
		if (null != direction) {
			...
			String [] pvalues = pvalueString.split(",");
			...
			Integer d;
			if (direction.equals(PhenotypeAnnotationType.decreased)) {
				d = 0;
			} else if (direction.equals(PhenotypeAnnotationType.increased)) {
				d = 1;
			} else {
				return null;
			}
			...
			try {
				return Float.parseFloat(pvalues[d]);
	...
	/**
	 * Return the effect size for the specified sex (or overall genotype if sex is null) for the
	 * decreased or increased change in mean, if available.
	 *
	 * Effect size for RR+ results is the percent of values that fall outside the reference range boundary
	 * decreased = low boundary, increased = high boundary
	 *
	 * The bounds are determined by looking at the range encompassing (default) 95% of the control data.
	 *
	 * @param sex pvalue for specific sex, null for "no sex" a.k.a. genotype effect
	 * @param direction decreased vs increased (decreased = low vs normal+high, increased = high vs low+normal)
	 * @return
	 */
	public Float getEffect(SexType sex, PhenotypeAnnotationType direction) {
		...
		if (null != direction) {
			...
			// Effect sizes come from PhenStat with a percent sign attached
			String [] effects = effectString.replace("%","").split(",");
			...
			try {
				return Float.parseFloat(effects[d]);
			} catch (NumberFormatException e) {
				return null;
			}
		} else {
	...
	/**
	 * Return the direction of the shift in mean for the specified sex (or overall genotype if sex is null)
	 *
	 * @param sex pvalue for specific sex, null for "no sex" a.k.a. genotype effect
	 * @return
	 */
	public PhenotypeAnnotationType getDirection(SexType sex) {

		PhenotypeAnnotationType direction = null;

		Float low = getEffect(sex, PhenotypeAnnotationType.decreased);
		Float high = getEffect(sex, PhenotypeAnnotationType.increased);

		// If the low effect is greater than the high effect, then the direction is decreased
		if (low != null && high != null) {
			if (low > high) {
				direction = PhenotypeAnnotationType.decreased;

				// If the low effect is less than the high effect, then the direction is decreased
			} else if (low < high) {
				direction = PhenotypeAnnotationType.increased;

				// Otherwise, we don't know. Probably equal
			} else {
				direction = null;
			}
		}
	...
	public String getGenotypeParameterEstimate() {
		return genotypeParameterEstimate;
	}

	public void setGenotypeParameterEstimate(String genotypeParameterEstimate) {
	...
	public String getGenderParameterEstimate() {
		return genderParameterEstimate;
	}

	public void setGenderParameterEstimate(String genderParameterEstimate) {
	...
	public String getGenderEffectPValue() {
		return genderEffectPValue;
	}

	public void setGenderEffectPValue(String genderEffectPValue) {
	...
	public String getGenderFemaleKoEstimate() {
		return genderFemaleKoEstimate;
	}

	public void setGenderFemaleKoEstimate(String genderFemaleKoEstimate) {
	...
	public String getGenderFemaleKoPValue() {
		return genderFemaleKoPValue;
	}

	public void setGenderFemaleKoPValue(String genderFemaleKoPValue) {
		this.genderFemaleKoPValue = genderFemaleKoPValue;
	}

	public String getGenderMaleKoEstimate() {
		return genderMaleKoEstimate;
	}

	public void setGenderMaleKoEstimate(String genderMaleKoEstimate) {
	...
	public String getGenderMaleKoPValue() {
		return genderMaleKoPValue;
	}

	public void setGenderMaleKoPValue(String genderMaleKoPValue) {
	...
	public boolean equals(Object o) {
		...
		StatisticalResultReferenceRangePlus that = (StatisticalResultReferenceRangePlus) o;
	...
	@Override
	public int hashCode() {
		int result = insertStatement != null ? insertStatement.hashCode() : 0;
	...
	public String toString() {
		return "StatisticalResultReferenceRangePlus{" +
