...
public abstract class AlleleProcessorAbstract implements ItemProcessor<Allele, Allele> {
    ...
    public Allele process(Allele allele) throws Exception {
        ...
        if (mgiFeatureTypes == null) {
            mgiFeatureTypes = cdabaseLoaderUtils.getOntologyTerms(DbIdType.MGI.intValue());
        }
    ...
    protected Allele setBiotypeMouseMutants(Allele allele) throws DataImportException {
        if (biotypeTm1a == null) {
            biotypeTm1a = cdabaseLoaderUtils.getOntologyTerm(DbIdType.MGI.intValue(), CdabaseLoaderUtils.BIOTYPE_TM1A_STRING);
            biotypeTm1e = cdabaseLoaderUtils.getOntologyTerm(DbIdType.MGI.intValue(), CdabaseLoaderUtils.BIOTYPE_TM1E_STRING);
        }
    ...
    protected Allele setGeneNullMeansAddWithdraw(Allele allele) throws DataImportException {
        ...
        if ((allele.getGene() == null) || (allele.getGene().getId().getAccession().trim().isEmpty())) {
            ...
            if (gene == null) {
                ...
                OntologyTerm biotype = cdabaseLoaderUtils.getOntologyTerm(DbIdType.Genome_Feature_Type.intValue(), CdabaseLoaderUtils.BIOTYPE_GENE_STRING);
