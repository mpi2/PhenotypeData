...
public class ImpcXmlFormatSpecimenLoader {
    ...
    private static final String analyticsQuery = "INSERT DELAYED INTO analytics_specimen_load (filename, colony_id, dob, baseline, strain, specimen_id, sex, zygosity, litter_id, impress_pipeline, production_center, phenotyping_center, project, mapped_project, status, message, additional_information) VALUES(?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
	...
	static Integer createdStrainCount = 0;
	...
	@Autowired
	uk.ac.ebi.phenotype.data.impress.Utilities impressUtilities;
	...
	private void initialize(String[] args) throws IOException, SQLException {
	...
	private void run() throws JAXBException, XMLloadingException, IOException, SQLException, KeyManagementException, NoSuchAlgorithmException {
		...
		ImitsRestClient imits = new ImitsRestClient();
		...
		for (CentreSpecimen specimenSet : specimenSets) {
			...
			for (Specimen specimen : specimenSet.getMouseOrEmbryo()) {
				...
				if ( !isBaseline && biologicalModelDAO.getLiveSampleBySampleIdAndOrganisationId(id, phenotypingCenter.getId()) != null ) {
					...
					if (allele == null && !isBaseline) {
                        ...
                        if (allele == null) {
                            try {
                                ...
                                allele = alleleDAO.getAlleleByAccession(alleleAccession);

                                logger.warn("Created allele {} for gene {} with accession {}", alleleSymbol, alleleGeneSymbol, alleleAccession);
...
// Not loading embryos at the moment
				...
				if (specimen instanceof Mouse) {
					...
					OntologyTerm efoTerm = impressUtilities.getStageTerm(((Embryo)specimen).getStage(), StageUnitType.valueOf(((Embryo) specimen).getStageUnit().value()));
					if (efoTerm==null) {
						logger.error("Could not get developmental stage EFO term for " + colonyID + " (animal ID: " + ((Embryo)specimen).getStage() + ") stage: " + allelicComposition + " stageUnit: " + ((Embryo) specimen).getStageUnit());
				...
				if (biologicalModel == null && biologicalModels.containsKey(bmKey)) {
					...
					Strain strain = getStrain(backgroundStrainName);
	...
	protected String getGeneticBackground(String background) throws StrainNotFoundException {
		...
		if (geneticBackgrounds.containsKey(background)) {
			...
			gb = StrainNames.getGeneticBackground(background, strainDAO);
	...
	protected Strain getStrain(String strainName) {

		Strain strain = null;

		if (strainName != null) {

			// LOG.debug("getStrain '" + strainName + "'.");

			// check existing Mapping table

			boolean wasMapped = StrainNames.STRAIN_MAPPING.containsKey(strainName);

			if (wasMapped) {
				strainName = StrainNames.STRAIN_MAPPING.get(strainName);
			}
			...
			if (strain == null) {
				...
				if (strain == null) {
					...
					if (strain == null) {
						...
						logger.error("Strain {} is not curated. Will insert it as a {} flagged strain", strainName, datasource);
						strain = new Strain();
						DatasourceEntityId id = new DatasourceEntityId();
						String datasourceAccName =  StringUtils.left(datasource.getShortName(), 4).toUpperCase();
						String strainAcc = (wasMapped) ? (datasourceAccName+"-MAPPED-" + createdStrainCount) : (datasourceAccName+"CURATE-" + createdStrainCount);
						id.setAccession(strainAcc);
						id.setDatabaseId(datasource.getId());
						strain.setId(id);
