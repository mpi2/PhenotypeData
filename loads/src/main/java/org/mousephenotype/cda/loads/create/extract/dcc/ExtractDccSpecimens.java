...
public class ExtractDccSpecimens implements CommandLineRunner {
    ...
    private void run() throws DataImportException {
        ...
        int relatedSpecimenUpdateCount = extractDccSqlUtils.updateRelatedSpecimenMinePk();
    ...
    private void insertSpecimen(Specimen specimen, CentreSpecimen centerSpecimen) throws DataImportException {
        ...
        // center
        long centerPk = extractDccSqlUtils.getCenterPk(centerSpecimen.getCentreID().value(), specimen.getPipeline(), specimen.getProject());
        if (centerPk < 1) {
            centerPk = extractDccSqlUtils.insertCenter(centerSpecimen.getCentreID().value(), specimen.getPipeline(), specimen.getProject());
        }

        // statuscode
        if (specimen.getStatusCode() != null) {
            extractDccSqlUtils.selectOrInsertStatuscode(specimen.getStatusCode().getValue(), specimen.getStatusCode().getDate());
        }

        // specimen
        Specimen existingSpecimen = extractDccSqlUtils.getSpecimen(specimen.getSpecimenID(), centerSpecimen.getCentreID().value());
        if (existingSpecimen != null) {
            ...
            specimen = extractDccSqlUtils.insertSpecimen(specimen);
            specimenPk = specimen.getHjid();
        }

        // embryo or mouse
        if (specimen instanceof Embryo) {
            extractDccSqlUtils.insertEmbryo((Embryo) specimen, specimenPk);
        } else  if (specimen instanceof Mouse) {
            extractDccSqlUtils.insertMouse((Mouse) specimen, specimenPk);
        } else {
        ...
        for (Genotype genotype : specimen.getGenotype()) {
            extractDccSqlUtils.insertGenotype(genotype, specimenPk);
        }

        // parentalStrain
        for (ParentalStrain parentalStrain : specimen.getParentalStrain()) {
            extractDccSqlUtils.insertParentalStrain(parentalStrain, specimenPk);
        }
        ...
        // center_specimen
        extractDccSqlUtils.insertCenter_specimen(centerPk, specimenPk);
        ...
        for (RelatedSpecimen relatedSpecimen : specimen.getRelatedSpecimen()) {
            extractDccSqlUtils.insertRelatedSpecimen(relatedSpecimen, specimenPk);
        }
    }
}