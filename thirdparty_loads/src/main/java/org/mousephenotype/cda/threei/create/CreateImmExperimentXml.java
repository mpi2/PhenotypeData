 ...
 */
public class CreateImmExperimentXml extends Create3iXmls implements CommandLineRunner {

    private String outFilename;
    private String inFilename;

    @Value("${n_imm_rows_per_mouse}")
    private int nRowsPerMouse;

    public static void main(String[] args) throws Exception {
        SpringApplication app = new SpringApplication(CreateImmExperimentXml.class);
    ...
    private void run() throws DataLoadException {
        ...
        while (reader.hasNext()) {
            ...
            for (String[] rowResult : rowsForMouse) {
                ...
                if (paramNo > 0 && paramNo <= 219) {
                    ...
                    if (paramNo > 131) {
                        // Simple parameter with '%' units
                        simpleParameter.setUnit("%");
                    }
                    simpleParameters.add(simpleParameter);
                 } else if (paramNo > 219 && paramNo <= 232) {
                    // ProcedureMetadata with no units
                    ...
                    logger.warn("Not expecting parameter with ID " + parameterImpressId + ". Row not processed");
                }
            }
        ...
        try {
            // Save to file
            XMLUtils.marshall(CreateImmExperimentXml.CONTEXT_PATH, centreProcedureSet, outFilename);
