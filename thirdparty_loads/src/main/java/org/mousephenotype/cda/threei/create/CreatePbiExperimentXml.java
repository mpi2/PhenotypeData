 ...
 */
public class CreatePbiExperimentXml extends Create3iXmls implements CommandLineRunner {

    private String outFilename;
    private String inFilename;

    @Value("${n_pbi_rows_per_mouse}")
    private int nRowsPerMouse;

    public static void main(String[] args) throws Exception {
        SpringApplication app = new SpringApplication(CreatePbiExperimentXml.class);
    ...
    private void run() throws DataLoadException {
        ...
        while (reader.hasNext()) {
            ...
            for (String[] rowResult : rowsForMouse) {
                ...
                if (paramNo >= 41 && paramNo <= 43)
                {
                    ...
                    if (paramNo >= 1 && paramNo <= 20) {
                        simpleParameter.setUnit("%");
                    } else if (paramNo >= 21 && paramNo <= 40) {
                        simpleParameter.setUnit("cells/ul");
                    }
        ...
        try {
            // Save to file
            XMLUtils.marshall(CreatePbiExperimentXml.CONTEXT_PATH, centreProcedureSet, outFilename);
